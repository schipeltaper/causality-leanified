...
Right: Flip a coin; if `heads' connect resistance to voltage source, if `tails' connect resistance to current source.
...
\begin{frame}{Structural Causal Models}
  SCMs extend L-CBNs and can deal with cycles.
  \begin{Def}[\ref{def:scm}]
...
\begin{frame}{Remarks}
  ...
  \bigskip
  Our definition of SCMs is unorthodox, as:
  \begin{enumerate}
    \item We allow for exogenous input variables;
...
We can construct solutions and (potential) outcomes in terms of solution functions of an SCM:
  \begin{Def}[\ref{def:scm_solution_functions}]
  We call a measurable function
    $g : \CXJ \times \CXW \to \CXV$ a \emph{solution function of SCM} $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ if it satisfies the structural equations:
...
\begin{frame}{Constructing solutions and potential outcomes}
  \begin{Rem}[\ref{rem:using_solution_functions}]
...
\begin{Prp}[\ref{prp:hard_interventions_commute}]
...
\begin{frame}{Decomposition}
  If we think about an SCM as modeling a ``system'', then we also obtain a model for a ``subsystem'' in the following way.
  ...
  Given an SCM $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ and a subset $V' \subseteq V$ of its endogenous variables, we define its \emph{submodel on $V'$} as the SCM
    $$M_{[V']} := \lt \Sin \cup (V \sm V'), V', W,\CX,P,f_{V'} \rt.$$
  \end{Def}
...
%  \begin{Prp}[\ref{prp:decomposing_composed_scms}]
%    $(M_{12})_{[V_1 \dcup W_1]} = M_1$ and $(M_{12})_{[V_2 \dcup W_2]} = M_2$.
%  \end{Prp}
...
\begin{frame}{Unique solvability}
  ...
  \end{Def}
  \begin{Thm}[\ref{thm:scm_unique_solvability}]
  \begin{enumerate}
...
%    \item If $M$ is uniquely solvable, then for all $\xJ\in\CXJ$, $M_{\doit(\XJ=\xJ)}$ is uniquely solvable.
    \item If $M$ is uniquely solvable, it has a unique Markov kernel
      $$P_{M}(X_V,X_W \mid \doit(X_J)) := (g,\id_{\CXW})_* \bigg( \bigg( \bigotimes_{w\in\Srnd} P_w(X_w) \bigg) \otimes \delta(X_J|X_J) \bigg)$$
...
\emph{linear} if all endogenous variables are real-valued and
each component of the causal mechanism is an affine combination of endogenous variables of the form
$$
...
For a linear SCM $M$ with $L \subseteq V$:
$M_{[L]}$ is uniquely solvable iff 
$I_{L}-B_{LL}(x_J,x_W)$ are invertible $\forall x_J \in \CX_J, x_W \in \CX_W$,
with solution function:
\small
  \begin{equation*}\begin{split}
...
%        \node at (0,3.2) {BCMs};
  ...
  \bigskip
  L-CBNs are equivalent to acyclic SCMs (Section~\ref{sec:interventional_equivalence_scms_cbns}).
  Simple SCMs are more expressive than L-CBNs, because they can model (sufficiently weak) causal cycles.
