...
a.s.-ness of the part that is integrated out. Perhaps turn the resit exam question of 20-21 on the chain rule into
a lemma.}
...
\begin{enumerate}
    \item $\displaystyle F \Perp^d_{G_{\doit(I_B,D)}} I_B  \given D$, and:
    \item $\displaystyle A \Perp^d_{G_{\doit(I_B,D)}} I_B  \given (B \cup F \cup D)$.
\end{enumerate}
...
By the first d-separation assumption we see by Proposition~\ref{prp:do-calculus} (rule 3) that we have the ``sure'' equality: $P(X_F|\doit(X_B)) = P(X_F)$.
By the second d-separation assumption we see by Proposition~\ref{prp:do-calculus} (rule 2) that we have a Markov kernel $P(X_A|X_F,\doit(X_B))$ that is also a version of $P(X_A|X_F,X_B)$.
