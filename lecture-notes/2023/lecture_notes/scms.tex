...
\Joris{Slightly improved notation for SCMs:
\begin{itemize}
  \item $M^\theta$ : SCM with parameter $\theta$
  \item $M_A$: marginalized SCM $M_{\sm (V\sm A)}$
  \item $M^{[A]}$: subSCM $M$ on $A$
  \item $M^\twin$: twinned SCM
  \item $g^{[A]}_B$: $X_B$ component of solution function of SCM $M^{[A]}$ 
  \item $M^{(1)}$, $M^{(2)}$, $M^{(3)}$, \dots, $M^{(a)}$: indexed SCMs
\end{itemize}
}
...
While causal Bayesian networks (with input nodes and latent variables) provide a powerful causal modeling class, there is an important aspect of causality that cannot be modeled with causal Bayesian networks, namely causal \emph{cycles}.
...
The class of (simple) SCMs to be introduced later is actually closed under the operation of merging variables.
...
\emph{exogenous parameters} $\theta\in\CX_\Theta$, which we may make explicit by writing
$f^{\theta}$ and $P^{\theta}$ instead, giving a parameterized SCM
$M^{\theta} = \lt \Sin, \Send, \Srnd, \CX, P^{\theta}, f^{\theta} \rt$.
The family $(M^{\theta})_{\theta \in \CX_\Theta}$ is then an SCM family.\footnote{In line with the convention in machine learning, the word ``model'' refers to a SCM with a fixed choice of the parameters, and ``model family'' to a family of models
...
\begin{Eg}[Linear regression model with fixed design for the effect in terms of its cause]
  ...
  $M^{(\alpha,\beta,\sigma^2)} = \lt \Sin, \Send, \Srnd, \CX, P_{\sigma^2}, f_{\alpha,\beta} \rt$ 
...
\begin{Def}[Solutions of an SCM]\label{def:scm_solutions}
  ...
  and $X$ satisfies the \emph{structural equations}
    \begin{equation}\label{eq:scm_structural_equations}
      \XV = f(\XJ, \XV, \XW) \qquad \text{$K(X|\XJ)$-a.s.}.
    \end{equation}
...
Consider an SCM with parameters $\alpha,\beta,\mu,\sigma \in \RN$, endogenous real-valued variables $X_1,X_2$, exogenous random real-valued variable $W_1$, structural equations
$$\begin{cases}
  X_1 = \alpha X_2 & \\
  X_2 = \beta X_1 + W_1, &
\end{cases}$$
and exogenous distribution $\C{N}(\mu,\sigma^2)$.
...
\begin{Eg}\label{ex:scms_formal}
  ...
  We can consider the following linear SCM families $M^{\theta} = \lt \Sin, \Send, \Srnd, \CX, P^\theta, f^\theta \rt$. 
  \begin{enumerate}
    \item $N$ causes $C$: (``Nobel prizes are celebrated with massive chocolate feasts'')\\
      ...
      For a given parameter $\theta$, the SCM $M^{\theta}$ has 
    ...
    \item $W$ causes $C$ and $N$, version 2: (``similar to version 1, but now the probability distribution of wealth is modeled''):\\
      ...
      \end{align*}
      For a given parameter $\theta$, the SCM $M^{\theta}$ has 
      unique solution function $g^\theta : x_W \mapsto (\alpha + \beta x_W, \gamma + \delta x_W) \in \C{X}_C \times \C{X}_N$ and
...
\Joris{This is experimental! In particular, it isn't clear to me how to do this in cyclic SCMs.
My guess would be that we'll end up ``scc splitting''. That would give an intuitive interpretation: we wait until the
first copy has equilibrated, then we may intervene on some variable in the scc, then we wait again until it has equilibrated.}

One versatile modeling tool for SCMs is the ``node splitting'' operation.
\begin{Def}
Given an SCM $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$.
We define the SCM $M_{\splt{w}}$ for $w \in \Srnd$ as
$M_{\splt(w)} := \lt \Sin \cup \{w'\}, \Send, \Srnd, \CX', P, f' \rt$
where $\CX' := \CX \times \CX_{w'}$ with $\CX_{w'} := \CX_w$, 
  ...
  $$
\end{Def}
In words, we add an endogenous copy of $w$.

We can do the same to split an endogenous variable $v$ (as long as it is not part of a cycle).
...
\begin{Def}[Taking a submodel of an SCM]\label{def:sub_scm}
  Given an SCM $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ and a subset $V' \subseteq V$ of its endogenous variables, we define its \emph{submodel on $V'$} as the SCM
  $$M^{[V']} := \lt \Sin \cup (V \sm V'), V', W,\CX,P,f_{V'} \rt.$$
\end{Def}
\begin{Rem}
  Note that this is just the intervened SCM $M_{\doit(V \sm V')}$.
\end{Rem}
...
\begin{Def}[Composing two SCMs]\label{def:compose_scm}
  Given two SCMs $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ and $\tilde M = \lt \tilde \Sin, \tilde \Send, \tilde \Srnd, \tilde \CX, \tilde P, \tilde f \rt$ and two subsets $C \subseteq \Sin \cap (\tilde \Send \cup \tilde \Srnd)$ and $\tilde C \subseteq (\Send \cup \Srnd) \cap \tilde \Sin$ satisfying the following conditions:
  \begin{enumerate}
...
\end{Def}
\begin{comment}
The composed SCM can again be decomposed into its composite submodels.
...
%  $$M_{12} := \lt (\Sin_1 \sm C_1) \dcup (\Sin_2 \sm C_2), \Send_1 \dcup \Send_2, \Srnd_1 \dcup \Srnd_2, \CX_{12}, P_{12} \rt,$$
  ...
  This means that composition followed by decomposition is interventionally equivalent to the original SCM.
\end{proof}
\end{comment}
