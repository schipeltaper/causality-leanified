...
\begin{enumerate}[label=8.\arabic*]
    \item Consider an L-CBN with variables $V=\{X, Y, Z\}$, $U=\{L\}$, $J=\emptyset$, graph $G$ as given by Figure \ref{fig:one}, with unknown Markov kernels $P(Y\given \doit(X, Z, L))$, $P(X\given \doit(Z))$, $P(Z\given \doit(L))$ and $P(L)$ and assume that an experimenter has estimated the following (conditional) distributions from observational data:
    \begin{align*}
    ...
    \begin{figure}[ht]
        ...
        \begin{tikzpicture}[scale=1, transform shape]
            ...
            % \node[ndout] (L) at (2.5, 1.5) {$L$};
            \draw[arout] (X) to (Y);
            \draw[arout] (Z) to (X);
            \draw[arout] (Z) to (Y);
            \draw[arlat,bend left] (Z) to (Y);
            % \draw[arout] (L) to (Z);
            % \draw[arout] (L) to (Y);
        \end{tikzpicture}
    ...
    Give the graph $G_{\doit(I_X)}$, show that the \emph{backdoor criterion} holds (Corollary \ref{cor:backdoor_adjustment}), and compute the \emph{average causal effect} $\beta := E(Y \given \doit(X=1)) - E(Y \given \doit(X=0))$ using backdoor adjustment (where you are allowed to assume the required absolute continuity).

    \begin{ungradedsolution}
        We have the graph $G_{\doit(I_X)}$:
        \begin{figure}[!ht]
            \centering
            \begin{tikzpicture}[scale=1, transform shape]
        ...
        in which the backdoor criterion holds:
        \begin{align*}
             & Z\Perp^d_{G_{\doit(I_X)}} I_X             \\
             & Y\Perp^d_{G_{\doit(I_X)}} I_X\given X, Z,
        \end{align*}
        so we have
        \begin{align*}
            E(Y\given \doit(X=1)) & = P(Y=1\given \doit(X=1))                                                  \\
                                  & = P(Y=1\given Z, \doit(X=1))\circ P(Z)                                     \\
                                  & = P(Y=1 | X=1, Z=0)P(Z=0)                                                  \\
                                  & + P(Y=1 | X=1, Z=1)P(Z=1)                                                  \\
        ...
        and similarly
        \begin{align*}
            E(Y\given \doit(X=0)) & = P(Y=1\given \doit(X=0))                                                  \\
                                  & = P(Y=1\given Z, \doit(X=0))\circ P(Z)                                     \\
                                  & = P(Y=1 | X=0, Z=0)P(Z=0)                                                  \\
                                  & + P(Y=1 | X=0, Z=1)P(Z=1)                                                  \\
    ...
    \begin{enumerate}[resume*]
        ...
        \item What would be your advice to a diseased patient of unknown sex?
    \end{enumerate}
    ...
    Suppose you believe that an L-CBN with causal graph as in Figure \ref{fig:Simpson}(i) applies, where $M$ denotes the sex of the patient (male/female).
    ...
    \begin{enumerate}[resume*]
        ...
        \item What would be your advice for a patient of unknown sex?
    \end{enumerate}
    ...
    know that most drugs don't change sex, but we could have used a slightly different story where the variable $M$ has a different interpretation
    ...
    \begin{ungradedsolution}
        \begin{enumerate}
            ...
            $p(R \given \doit(D))$ using the adjustment formula in \ref{Simpson_1}. As $p(R|D=1,M) < p(R|D=0,M)$ for both sexes, the same will hold for a mixture:
    ...
    \begin{figure}[!ht]
        ...
        \begin{tikzpicture}
            ...
            \draw[arout] (W1) to (S);
        \end{tikzpicture}
        \caption{In this causal graph, $U$ is a latent variable and $S$ is a selection variable.
    ...
    In the following exercises, you will use the do-calculus to show that this does not pose an actual problem. You may assume all absolute continuities that are necessary for applying the rules of do-calculus.
