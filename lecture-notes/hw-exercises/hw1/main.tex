% !TEX root = hw1-answers-graded.tex
...
\section*{Topic: Basic causality}

\begin{enumerate}[label=1.\arabic*]
    \item % ($3 \times 3 \times 0.5$ points)
    The following fictional news headlines state a correlation and suggest a causal relationship. However, we know that correlations do not always imply the ``obvious'' causation. For each headline, come up with three different hypotheses about the causal relationships, potentially including variables that are not mentioned in the headlines. Describe the hypotheses briefly in words and with a causal graph, such as the ones given in ``Explanation \ref*{explanation:nobel_chocolate}'' in the lecture notes.
    ...
    \begin{ungradedsolution}
        ...
        \begin{enumerate}
            \item The dosage is determined by the country, and half a dose is more effective: country $\to$ dose $\to$ immunization. (2) the populations differed (e.g. younger in country A): country $\to$ age $\to$ immunization. (3) the virus in B is more resistant than the virus in country A: country $\to$ virus $\to$ immunization.
            \item Group A is more predisposed to commit crime: group $\to$ predisposition to crime $\to$ crime $\to$ arrests. (2) Group A is more heavily policed so that crimes more often lead to arrests: group $\to$ policing $\to$ arrests. (3) Group A is poorer and poverty causes crime: group $\to$ poverty $\to$ crime $\to$ arrests. (4) Group A are those with a prior conviction: group $\leftarrow$ predisposition to crime $\to$ crime $\to$ arrests.
            \item Working hard on high school prepares the students well for career success: HS grades $\leftarrow$ worked hard at HS $\rightarrow$ good career. (2) Students with well-educated/rich parents do well in high school and also in their career: HS grades $\leftarrow$ parents $\to$ good career. (3) employers hire from certain schools that only give high grades: HS grades $\leftarrow$ school $\to$ good career.
        \end{enumerate}
    \end{ungradedsolution}

    \item % ($3 \times 2 \times 0.5$ points )
    ...
    \item % ($1+1$ points)
    In the brains of Alzheimer's disease patients, amino acids called amyloid beta plaque are found. On June 7th, 2021, the FDA gave accelerated approval to the Alzheimer's drug Aduhelm, releasing the following statement:
    ...
    \begin{gradedsolution}
        \begin{enumerate}
            \item \points{1} $E = \{(T, \beta), (\beta, O)\}$, or $E = \{(T, \beta), (\beta, O), (T, O)\}$
            \item \points{1} $E' = \{(T, \beta), (O, \beta)\}$. Other correct answers exist.
        \end{enumerate}
    \end{gradedsolution}

\end{enumerate}

\section*{Optional Measure Theoretic Probability practice exercises}
\begin{enumerate}[resume*]
    \item % ($1+1$ points)
    \begin{enumerate}
        \item Let $X$ have a uniform distribution on $\{-M, -M+1, \dots, N-1, N\}$ where $M,N \in \mathbb{N}$ and $M>N$. Compute the probability mass function of the absolute value $Y=|X|$.
        \item Let $X$ have a standard Gaussian distribution $\mathcal{N}(0,1)$. Furthermore, let $Y = \exp (X)$. Compute the density of $Y$ w.r.t.\ the Lebesgue measure.
    \end{enumerate}
    \begin{ungradedsolution}
        \begin{enumerate}
            \item Since there are $M+N+1$ values for $X$, on the original domain each value has a probability mass $\frac{1}{M+N+1}$. Then for $Y$ we calculate the probability density $P[Y=y]$ by taking the push-forward measure of single elements: $P^Y(y) = P^X(Y^{-1}(y)) = \frac{|Y^{-1}(y)|}{M+N+1}$. That is, we calculate the pre-images of each element of $Y$, and calculate the probability of this set using the original law $P^X$. For $1, \dots, N$ two values map to these values. For $0$ and $N < Y \leq M$, only one value maps to these values, so:
            \begin{equation*}
                P[Y=y] = \begin{cases}
                ...
                \end{cases}
            \end{equation*}
            \item Let $p_X(x)=\frac{1}{\sqrt{2\pi}} e^{-x^2/2}$ be the pdf of the standard normal distribution. We can directly apply Corollary \ref*{lebesgue-density-transformation} (from the appendix) to obtain:
            \begin{equation*}
    ...
    \item % (2+2pts)
    A discrete die has as sample space the six sides $\Xcal=\{a, b, c, d, e, f\}$ with uniform measure $P_D$ with probability mass function $p_D$. Consider as assiciated value the random variable $V_D:\Xcal \to \R$ with:
    \begin{equation*}
        V(a)=1, V(b)=2, V(c)=3, V(d)=4, V(e)=5, V(f)=6.
    \end{equation*}
    A continuous die has as sample space the unit interval $[0, 1]$ with uniform measure $P_C$ with probability density function $p_C$. The associated value is a random variable $V_C:[0,1] \to \R : x \mapsto 5 x + 1$. Using the measure theoretical notation introduced in Lecture 2, compute the expected value of the discrete and continuous dice.
    ...
    \item %  (1+2+1+1 points)
    \begin{enumerate}
        \item \label{ex:mble_comp_is_mble} Consider spaces $A$, $B$ and $C$ with respective sigma algebras $\mathcal{F}, \mathcal{G}$ and $\mathcal{H}$. Show that if the function $f: A \rightarrow B$ is $\mathcal{F}$-$\mathcal{G}$-measurable and the function $g: B \rightarrow C$ is $\mathcal{G}$-$\mathcal{H}$ measurable, then their composition $g \circ f$ is $\mathcal{F}$-$\mathcal{H}$-measurable.
    ...
    \begin{enumerate}
        \item % (1)
        Let $X: \Omega \rightarrow \mathbb{R}$ be a simple function. Show from first principles that we have $\mathbb{E}[cX] = c\mathbb{E}[X]$ (i.e.\ without just applying the linearity of the integral).
        \item % (Bonus)
        Generalize this statement to the case where $X : \Omega \rightarrow \mathbb{R}$ is a random variable.
    \end{enumerate}
    \begin{ungradedsolution}
        \begin{enumerate}
            \item Suppose $X(\omega) = \sum_{n=1}^N a_n \I_{A_n}(\omega)$ is simple. Then $cX$ is also simple and
            \begin{equation*}
                \mathbb{E}[cX] = \int cX \,dP = \sum_{n=1}^N c a_n P(A_n) = c \sum_{n=1}^N a_n P(A_n) = c \mathbb{E}[X]
            \end{equation*}
            \item Now, suppose $X$ is positive (but not necessarily simple) and $c>0$. Then $cX$ is also positive, and
            \begin{align*}
            ...
            \end{align*}
            Then, for arbitrary random variables and $c>0$ we have
            \begin{equation*}
