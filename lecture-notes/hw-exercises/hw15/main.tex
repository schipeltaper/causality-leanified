...
\begin{document}
\flushleft\textbf{Topic: Causal Discovery and Conditional Independence Testing}

\begin{enumerate}[label=15.\arabic*]
	\item %(2 points)
	Two key assumptions of Randomized Controlled Trials with treatment variable $C$ and outcome variable $X$ are:
	\begin{enumerate}
	...
	Describe a situation in which the first assumption is not valid. Do the same for the second assumption.
	...
	\item %(3 points)
	Prove Proposition \ref*{prp:y-structures} on Y-structures by using the global Markov property and the faithfulness assumption, and showing that the only (cyclic or acyclic) graphs that are compatible with the observed
	...
	\item Make the programming exercise, available via the following link:

	\bigskip
	\href{https://colab.research.google.com/drive/1uB64Ir9vHT9x_2qMOb3CMHse93Gl_Fdm?usp=sharing}{https://colab.research.google.com/drive/1uB64Ir9vHT9x\_2qMOb3CMHse93Gl\_Fdm?usp=sharing}

	\bigskip
	Please, carefully read the instructions at the beginning of the assignment.
