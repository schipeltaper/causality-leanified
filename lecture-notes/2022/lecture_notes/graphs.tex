...
\begin{Not}
    ...
    \end{enumerate}
    The star stands for a placeholder to mean: ``arrow head or tail''.
    If $v_1 \sus v_2 \in G$ then we call $v_1$ and $v_2$ \emph{adjacent in $G$}.
\end{Not}
...
%      \[v=v_0 \sus v_1 \sus  \cdots \sus v_{n-1} \sus v_n=w\]
    ...
    \item[] In the definition of walk the same node may appear multiple times.
   ...
   % \item[] Directed walks exclude the trivial walk per definition.
    ...
    \item A \emph{collider walk} from $v$ to $w$ in $G$ is of the form:
        ...
        for some $n \ge 0$, where all nodes in between $v$ and $w$ have two arrow heads pointing towards them (a.k.a.\ collider).
        Note that for $n=1$ this reads: $v\sus w \in G$.
    \item A walk is called \emph{path} if no node occurs more than once.
    \item A \emph{fork} from $v$ to $w$ in $G$ is a walk of the form:
   ...
   %             \[v=v_0 \hut v_1 \hut  \cdots \hut v_{k-1} \huh v_k  \tuh \cdots \tuh v_{n-1} \tuh v_n=w,\]
        ...
        both endnodes have an arrow head pointing towards them. 
          ...
          %(Usually forks are only considered for $v \neq w$.)
  \end{enumerate}
\end{Def}
...
The following proposition expresses the existence of a fork with a source in terms of ancestral relations in intervened graphs.
\begin{Prp}\label{prp:forks_alternative}
  Let $G = \lt J, V, E \rt$ be a CDMG. For $v, w, c \in V$:
  there exists a fork between $v$ and $w$ in $G$ with source $c$ if and only if $v \ne w$ and $c \in \Anc^{G_{\doit(w)}}(v) \sm \{v\}$ and $c \in \Anc^{G_{\doit(v)}}(w) \sm \{w\}$.
\end{Prp}
\begin{proof}
  A fork between $v$ and $w$ with source $c$ is a walk in $G$ of the form $v \hut \cdots \hut c \tuh \cdots \tuh w$, where $v,w,c$ are distinct and both $v$ and $w$ appear exactly once on the walk.
  This shows ``$\implies$''. For the other implication, note that $c \in \Anc^{G_{\doit(w)}}(v) \sm \{v\}$ implies that there is a non-trivial directed walk from $c$ to $v$ that does not pass through $w$. 
  Similarly, $c \in \Anc^{G_{\doit(v)}}(w) \sm \{w\}$ implies that there is a non-trivial directed walk from $c$ to $w$ that does not pass through $v$. 
  The concatenation of the two walks $v \hut \cdots \hut c \tuh \cdots \tuh w$ is then a fork between $v$ and $w$ with source $c$.
\end{proof}
...
\end{scope}
\end{tikzpicture}
\caption{The CADMG from Figure \ref{fig:cadmg}  after a node splitting intervention on $v_5$ and $v_7$.}
...
In this subsection we introduce \emph{node splitting interventions}. 
...
\begin{Def}[Node splitting intervention on CDMGs]
  \label{def:G_node-splitting_intervention}
    Let $G=(J,V,E,L)$ be a CDMG and $W \ins V$ a subset of the output nodes.
    The \emph{single-world intervention graph (SWIG)} w.r.t.\ $W$ of $G$ is the CDMG: 
  ...
  \begin{enumerate}[label=\roman*.)]
      ...
      \item $V_{\swig(W)} := (V \sm W) \dcup W^o$, %which could be identified with $V$ again if we want to make the identification
          $W=W^o$.
      \item $E_{\swig(W)} := \lC v^i_1 \tuh v_2^o \st v_1 \tuh v_2 \in E  \rC$,
      \item $L_{\swig(W)} :=\lC v_1^o \huh v_2^o \st v_1 \huh v_2 \in L \rC$.
  \end{enumerate}
  where we turn all nodes of $W^i$ into input nodes, removing all edges with arrow heads at nodes of $W^i$,
  and we turn all nodes of $W^o$ into output nodes, removing all their outgoing edges.
\end{Def}
...
\begin{Rem}
    If a CDMG $G=(J,V,E,L)$  is acyclic then also $G_{\swig(W)}$ is acyclic.
...
\begin{Lem}[Two disjoint node splitting interventions commute]
   Let $G=(J,V,E,L)$ be a CDMG and $W_1, W_2 \ins V$ two disjoint subsets of the output nodes from $G$. 
   Then the CDMG obtained from first node splitting on $W_1$ and then node splitting on $W_2$ is the same CDMG that arises from first node splitting on $W_2$ and then node splitting on $W_1$:
   \[ \lp G_{\swig(W_1)} \rp_{\swig(W_2)} =  \lp G_{\swig(W_2)} \rp_{\swig(W_1)}  =  G_{\swig(W_1 \dcup W_2)}.   \]
\end{Lem}


\begin{Lem}[Disjoint hard interventions and node splitting interventions commute]
   Let $G=(J,V,E,L)$ be a CDMG and $W_1 \ins J \cup V$ and $W_2 \ins V$ two disjoint subsets of nodes from $G$. 
   Then the CDMG obtained from first hard intervening on $W_1$ and then node splitting on $W_2$ is the same CDMG that arises from first node splitting on $W_2$ and then hard intervening on $W_1$.
...
\begin{Rem}
    ...
    then first hard intervening on $w$ turns $w$ into an input node, for now indicated as $w^i$, and a node splitting intervention (if we would define it for input nodes) would not change $w^i$.
...
\begin{tikzpicture}[scale=1, transform shape]
    ...
    \draw[arlat, bend left] (v1) to (v6);
\end{tikzpicture}
  \caption{The CADMG from Figure \ref{fig:cadmg} with additional intervention node $I_{v_7}$ at node $v_7$. \Joris{Here it would be helpful to also add an intervention node for $v_2$.}}
\end{figure}
...
\begin{Rem}
    ...
    by ordering all the $I_w$ nodes first.
\end{Rem}
...
\begin{Lem}[Adding intervention nodes commutes with disjoint node splitting interventions]
     Let $G=(J,V,E,L)$ be a CDMG and $W_1, W_2 \ins V$ two disjoint subsets of nodes from $G$.
    Then the CDMG that arises from first introducing interventions nodes $I_{W_2}$ and then splitting the nodes from $W_1$ is the same as the CDMG that arises from first splitting the nodes from $W_1$ and then introducing the intervention nodes $I_{W_2}$:
...
\begin{Def}[Marginalization a.k.a.\ latent projection on CDMGs]\label{def:G_marginalization}
  ...
  \begin{enumerate}[label=\roman*.)]
      ...
      \item $L^{\sm W}$ consists of all bi-directed edges $\ul{v} \huh \ol{v}$ with 
          ...
          for which there exists a fork in $G$:
          \[\ul{v} \hut w_1 \hut \cdots \hut w_{k-1} \hus w_k \tuh \cdots \tuh w_{n-1} \tuh \ol{v}, \]
          where all intermediate nodes $w_1,\dots,w_{n-1} \in W$ (if any).
      %\item $L^{\sm W}:= \lC \ul{v} \huh \ol{v}\,|\, \ul{v},\ol{v} \in V \sm W: 
      %    \exists \text{ fork: } \ul{v} \hut w_1 \cdots w_{n-1} \tuh \ol{v} \in G 
      %    \text{ with } w_1,\dots,w_{n-1} \in W \rC$,
  \end{enumerate}
\end{Def}

\begin{Rem}[Marginalization preserves ancestral relations, forks and acyclicity]\label{rem:marg_preserves_ancestors_forks_acyclicity}
    \begin{enumerate}
        \item For $v_1,v_2 \in G$ with $v_1,v_2 \notin W$ we have the equivalence:
            \[ v_1 \in \Anc^G(v_2)\quad \iff \quad v_1 \in \Anc^{G^{\sm W}}(v_2).   \]
          \item For $v_1,v_2 \in G$ with $v_1,v_2 \notin W$ there is a fork between $a$ and $b$ in $G$ if and only if there is a fork between $a$ and $b$ in $G^{\sm W}$.\label{rem:marg_preserves_forks}
...
\end{Rem}
\begin{proof}
  We prove \ref{rem:marg_preserves_forks}.
  Let \[v_1=w_0 \hut w_1 \hut  \cdots \hut w_{k-1} \hus w_k  \tuh \cdots \tuh w_{n-1} \tuh w_n=v_2,\]
     be a fork in $G$. If one marginalizes out a single node $u$ that is not on the fork,
     then the same fork exists in $G^{\setminus\{u\}}$. If one marginalizes out a single node
     $u$ that appears on the fork (but that is not an endpoint) one obtains again a fork in $G^{\setminus\{u\}}$. 
...
\begin{Lem}[Marginalization and intervention commute]
     ...
     % \[G(V\sm (W_1 \cup W_2)|\doit(W_1 \cup J))=G_{\doit(W_1)}^{\sm W_2}.\]
      A similar statement holds for marginalizations and adding intervention nodes.
\end{Lem}
...
\begin{Rem}
  ...
  \begin{enumerate}[label=\roman*.)]
    ...
    \item all colliders on $\pi$ are in $\Anc^G(C)$, and
    \item all (non-endpoint) non-colliders $v_k$ on $\pi$ are either not in $C$ or all of its adjacent children, $v_{k\pm 1}$,
        on $\pi$ lie in its strongly connected component $\Sc^G(v_k)$. \Joris{the last part ``all of its neighboring children on $\pi$ lie in its strongly connected component'' is equivalent to ``unblockable'' (see Definition~\ref{def:unblockable_noncollider}); we might move that definition to here for convenience.}
  \end{enumerate}
\end{Rem}



\begin{Def}[$\sigma$-separation]
    ...
    if every walk from a node in $A$ to a node in $J \cup B$ (sic!) is $C$-$\sigma$-blocked by $C$.
...
\begin{Prp}\label{prp:sigma_opens}
  ...
  equivalent:
  \begin{enumerate}
      \item there exists a $C$-$\sigma$-open \emph{path} between $v_1$ and $v_2$ in $G$;
      \item there exists a $C$-$\sigma$-open \emph{walk} between $v_1$ and $v_2$ in $G$;
      \item there exists a $C$-$\sigma$-open \emph{walk} between $v_1$ and $v_2$ in $G$ such that all its colliders lie in $C$ (and not just in $\Anc^G(C)$).
  \end{enumerate}
\end{Prp}
...
\begin{Prp}\label{prp:sigma_opens-2}
  ...
  \begin{enumerate}
      ...
      \item there exists a $C$-$\sigma$-open \emph{walk} between $w_1$ and $w_2$ in $G$ such that all its colliders lie in $C$.
  \end{enumerate}
...
\begin{Def}[d-blocked walks]\label{def:d-blocking}
      ...
      $\pi$ is d-blocked by $C$ if it either contains a non-collider in $C$ or a collider not in $\Anc^G(C)$.
...
\begin{Def}[d-separation]
    ...
    if every walk from a node in $A$ to a node in $J \cup B$ (sic!) is $C$-d-blocked by $C$.
        \item If that property does not hold we will write:
            \[ A \nPerp^d_G B \given C.\]
    \end{enumerate}
\end{Def}
