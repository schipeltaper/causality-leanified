...
\begin{Rem}[Mutual independence for families of random variables]
    \begin{enumerate}
        \item If $I$ is a countable index set one can define the countable product measure: $\bigotimes_{ i \in I} P(X_i)$, by the theorem of Ionescu-Tulcea, see \cite{Kle20} Thm.\ 14.35 and
    Cor.\ 14.36, which is determined by its finite marginals. 
...
\Joris{Perhaps explicitly state the obvious: that $\displaystyle  X \Indep_{K(W|T)} T \given Z$
and for every probability distribution $Q(T) \in \Pcal(\Tcal)$:
        \[ X \Indep_{K(W|T)\otimes Q(T) } T,Y \given Z.\]
    also implies iv), and hence $\displaystyle X \Indep_{K(W|T)} Y \given Z.$
    ...
    Note that $\displaystyle  X \Indep_{K(W|T)} T \given Z$ is equivalent to the existence of $Q$ s.t.\ $K(X,Z|T) = Q(X|Z)\otimes K(Z|T)$. By the way, Patrick wrote on my BB the slightly different statement that
   $\displaystyle  X \Indep_{K(W|T)} T \given Z$
and for every probability distribution $Q(T) \in \Pcal(\Tcal)$:
        \[ X \Indep_{K(W|T)\otimes Q(T) } Y \given Z,T.\]
        implies that $\displaystyle X \Indep_{K(W|T)} Y,T \given Z.$
        Which one would be required? I am still somewhat confused. Yet another option would be:
   $\displaystyle  X \Indep_{K(W|T)} T \given Z$
and for every probability distribution $Q(T) \in \Pcal(\Tcal)$:
...
Consider a linear Gaussian Markov kernel $P(X,Y,Z|T)$, which is then given by a density of the form:
\begin{align*}
...
\end{align*}
The conditional $P(X|Y,Z,T)$ is then also a linear Gaussian Markov kernel and of the form:
\begin{align*}
...
with density:
\begin{align*}
    q(x|z) &= \Ncal\lp x \st \mu_{X|Z=z}, \Sigma_{X|Z} \rp,\\ 
