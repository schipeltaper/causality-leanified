...
\begin{enumerate}
\item ($3 \times 1$ points) In this exercise, we want to gain a more intuitive understanding of the backdoor criterion. 
...
\item ($15 \times 1/5$ points) You are investigating the effectiveness of a drug against a deadly disease.
...
\begin{enumerate}[resume*]
  \item \label{Simpson_1} Apply the back-door criterion to obtain a formula that expresses $P(R \given \text{do}(D))$ in terms of observable quantities (i.e., in terms of marginal or conditional distributions where the \emph{do}-operator does not appear).
  \item Is $P(R \given \text{do}(D)) = P(R \given D)$ in this case?
...
\begin{enumerate}[resume*]
  \item Again, use the back-door criterion to express $P(R \given \text{do}(D))$ in terms of observable quantities.
  \item Is $P(R \given \text{do}(D)) = P(R \given D)$ in this case?
...
\begin{enumerate}[resume*]
  \item Invent an interpretation of $M$ and the two latent variables $L_1,L_2$ yourself that could match the causal model depicted in Figure \ref{fig:Simpson}(iii).
  \item Express $P(R \given \text{do}(D))$ in terms of \emph{observable} quantities.
  \item Is $P(R \given \text{do}(D)) = P(R \given D)$ in this case?
...
\item ($4\times 1$ points) Selection bias is a problem in the inference of observational and causal relationships.
...
\begin{figure}
  \centering
  \includegraphics[width=0.4\textwidth]{bareinboim}
...
\begin{itemize}
  ...
  \begin{enumerate}
    \item Show that $P(Y | \text{do}(X)) = P(Y | S, \text{do}(X))$.
    \item Show that $P(Y | S, \text{do}(X)) = P(Y | S, W_2, \text{do}(X)) \circ P(W_2 | S)$.
    \item Show that  $P(Y | S, W_2, \text{do}(X)) = P(Y | S, W_2, X)$.
...
\begin{gradedsolution}
  \begin{enumerate}
    ...
    For step 3, one can directly apply Remark \ref*{rem:markov-kernel-product-and-composition} from the lecture notes on the connection between product Markov kernels and compositions of Markov kernels.
