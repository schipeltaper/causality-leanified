% !TEX root = hw15-answers.tex
\chead{\textbf{Exercises week 15}}
\begin{document}
...
\begin{enumerate}
	\item \textbf{(1 point)} Provide the $\sigma$-independence model of $G$, $\IM_\sigma(G)$. It's okay to only consider singleton sets $A, B\subseteq V$ (where $A,B$ relate to Definition \ref*{def:independence_model}).
	...
	\item \textbf{(1 point)} List the combinations of nodes $\{i,j\}$ with $i,j\in V$ such that there is a $\sigma$-inducing path between $i$ and $j$ in all graphs $\tilde{G}$ that are $\sigma$-Markov equivalent to $G$.
	...
	\item \textbf{(1 point)} Draw the unique skeleton $\skel(\DPAG(G))$ and the representing $\DPAG(G)$ that is most informative of the (non-)ancestral relations. (With \emph{most informative}, we mean that there are no circles as edge marks that can be oriented by, for example, applying Proposition \ref*{prp:from_dmg_to_dpag}.)
	\item \textbf{(3 points)} By hand, 'run' FCI on $\IM_\sigma(G)$. Show for each step of the algorithm which rules apply and why they apply, provide the intermediate graph (if applicable), and finally, give the output $\FCI(\IM_\sigma(G))$.
	\item \textbf{(1 point)} List the ancestral and non-ancestral relations that can be read off from the output of FCI, and compare these conclusions with Proposition \ref*{prp:y-structures} (and in particular, the conclusions that can be drawn from Figure \ref*{fig:Ystruct} of the lecture notes).
...
\begin{gradedsolution}
	\begin{enumerate}
		...
		\item By Proposition \ref*{prp:inducing_path_properties} iii), the $\sigma$-inducing paths in $G$ are given by all nodes $\{i,j\}$ that do not occur on the left hand side of the $\sigma$-separations in answer (a). Hence, the $\sigma$-inducing paths are between the nodes: $\{X_1, X_3\}, \{X_2, X_3\}, \{X_3, X_4\}$.
		...
		\item Combining definitions \ref*{def:skeleton} and \ref*{def:dpag_represents_dmg}, we have $\skel(\DPAG(G))$ given by the graph containing a bicircle edge $i \ouo j$ if and only if there is a $\sigma$-inducing path between $i$ and $j$. Hence, $\skel(\DPAG(G))$:
		\begin{center}
			\begin{tikzpicture}
				\node[ndout] (X1) at (-1,0) {$X_1$};
				\node[ndout] (X2) at (1,0) {$X_2$};
			...
			\end{tikzpicture}
		\end{center}
		We can orient the edges using Proposition \ref*{prp:from_dmg_to_dpag} to obtain the following $\DPAG(G)$:
		\begin{center}
			\begin{tikzpicture}
				\node[ndout] (X1) at (-1,0) {$X_1$};
				\node[ndout] (X2) at (1,0) {$X_2$};
		...
		\begin{enumerate}[label=\arabic*)]
			...
			\end{center}
		\end{enumerate}
		\item Using Definition \ref*{def:dpag_represents_dmg} we infer the ancestral relation $X_3 \in \Anc_G(X_4)$, and the non-ancestral relations $X_4 \notin \Anc_G(X_3), X_3 \notin \Anc_G(X_1)$ and $X_3 \notin \Anc_G(X_2)$.
