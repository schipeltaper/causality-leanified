...
\subsection{Marginal Independence of a Random and a Non-Random Variable}
...
$$K((X_n)_{n=1}^N \mid (c_n)_{n=1}^N) = \bigotimes_{n=1}^N K(X_n \mid C_n=c_n),$$
where each $K(X_n \mid C_n)$ is a copy of the (``true'' but unknown kernel) $K(X \mid C)$.
...
sampled from some distribution. This extends the possible experimental designs that
...
$$\CX_\Theta := %\{ \theta \in \prod_{z\in\C{Z}} [0,1] : \sum_{z\in\C{Z}} \theta_z = 1\} \times
...
in terms of the parameters as $H_0 : \theta \in \CX_\Theta^0$, where we introduced the restricted parameter space
...
%\PF{Don't we need first a justification of some sort that conditional maximal likelihood is a valid fitting principle?} 
%\Joris{Do you have one? It appears to me to be the same principle as maximum likelihood, except that we have kernels instead of distributions for each data point?}
We first write down the ``conditional'' likelihood:
$$\theta \mapsto K\big((X_n)_{n=1}^N \mid (c_n)_{n=1}^N, \theta\big)
= \prod_{n=1}^N \theta_{X_n|c_n} 
= \prod_{x\in\C{X}} \prod_{c\in\C{C}} \theta_{x|c}^{N_{x|c}}$$
where we used the counts as a sufficient statistic.
...
Under $H_0$, we can write $\theta_{x|c} = \theta_{x|\ast}$ for some $\theta_{x|\ast} \in \CX_\Theta^0$, 
and the likelihood simplifies:
$$\theta \in \CX_\Theta^0 \implies \prod_{x\in\C{X}} \prod_{c\in\C{C}} \theta_{x|\ast}^{N_{x|c}} = 
...
\begin{equation}\label{eq:ext_G_test_statistic}
  \frac{\sup_{\theta\in\CX_\Theta^0 \cup \CX_\Theta^1} K((X_n)_{n=1}^N \mid (c_n)_{n=1}^N, \theta)}{\sup_{\theta\in\CX_\Theta^0} K((X_n)_{n=1}^N \mid (c_n)_{n=1}^N, \theta)}
...
$X \Indep_{P(X,C)} C$ with two random variables $X,C$.
The likelihood ratio test statistic is defined as 2 times the natural logarithm of this ratio:
$$G_N := 2 \log \frac{\sup_{\theta\in\CX_\Theta^0 \cup \CX_\Theta^1} K((X_n)_{n=1}^N \mid (c_n)_{n=1}^N, \theta)}{\sup_{\theta\in\CX_\Theta^0} K((X_n)_{n=1}^N \mid (c_n)_{n=1}^N, \theta)} = 2 \sum_{x\in\C{X}} \sum_{c\in\C{C}} N_{x|c} \log \frac{N_{x|c} N}{N_{x|+} N_{+|c}}$$
...
assumptions on the sequence $c_1,c_2,\dots$ we can apply the standard $G$ test and ignore the non-random nature of the $c_n$'s.

We will start with rewriting the test statistic. We introduce the space
$$\CX_{\Theta_C} := \{ \gamma \in \prod_{c\in\C{C}} [0,1] : \sum_{c\in\C{C}} \gamma_c = 1 \}.$$
%\PF{It seems to me here that we are making the implicit assumption that we can model the sequence $c_1,\dots,c_n,$ as i.i.d.\ samples with an unknown distribution $\gamma$.}
%\Joris{We are not, the notation is just reminiscent of that.}
Consider now the function
\begin{equation}\label{eq:G_test_statistics_limit}
  \begin{split}
    g : \CX_\Theta \times \CX_{\Theta_C} : (\theta,\gamma) \mapsto
    &:= \sum_{c\in\C{C}} \gamma_c \sum_{x\in\C{X}} \theta_{x|c} \log \frac{\theta_{x|c}}{\sum_{c\in\C{C}} \gamma_c \theta_{x|c}} \\
    & = \sum_{c\in\C{C}} \gamma_c \mathrm{KL}\Big(\theta_{X|c} \,\Vert\, \sum_{c\in\C{C}} \gamma_c \theta_{X|c}\Big),
\end{split}\end{equation}
%\PF{So we are using $\E_{\gamma(C)} \KL(P(X|C) \| P(X|C') \circ \gamma(C'))$? }\Joris{I guess you could also write it like that indeed.}
...
$$\mathrm{KL}(P \,\Vert\, Q) := \sum_{x\in\C{X}} P(X=x) \log \frac{P(X=x)}{Q(X=x)}$$
...
Because
  $$N_{x|c} = \sum_{\substack{n=1 \\ c_n=c}}^N \I_{x}(X_n),$$
...
central limit theorem to conclude that
  \begin{equation}\label{eq:multinomial_clt}
    \sqrt{N_{+|c}} (\hat\theta_{x|c} - \theta_{x|c}) \leadsto \C{N}(0,\Sigma)
  \end{equation}
...
\begin{Lem}\label{lem:g_derivatives_H0}
  Let $\theta \in \CX_\Theta^0$ be positive, i.e., such that there exists a $\delta > 0$ with $\theta_{x|c} \ge \delta$ for all $x \in \C{X}, c \in \C{C}$.
  Let $\gamma \in \CX_{\Theta_C}$.
  For $\hat\theta$ with $\lVert \hat\theta - \theta \rVert$ sufficiently small and $\hat\theta \stackrel{P}{\to} \theta$,
  \begin{equation}
...
The Kullback-Leibler divergence has the important property that $\mathrm{KL}(P \,\Vert\, Q) \ge 0$ and $\mathrm{KL}(P \,\Vert\, Q) = 0 \iff P = Q$ for all $P,Q \in \C{P}(\C{X})$. 
Together with the definition \eref{eq:G_test_statistics_limit}, 
this immediately implies that under $H_0$, $g(\theta,\gamma) = 0$ for all $\gamma \in \CX_{\Theta_C}$.
...
the gradient vanishes for all $\gamma\in\CX_{\Theta_C}$.
...
\begin{Prp}\label{prp:ext_G_test_asymptotics_H0}
  Let $\theta \in \CX_\Theta$ be positive, i.e., such that $\theta_{x|c} > 0$ for all $x \in \C{X}, c \in \C{C}$.
...
\end{Prp}
\begin{proof}
  We first note that $\hat\theta \stackrel{a.s.}{\to} \theta$ (for any $\theta \in \CX_\Theta$) by applying the strong law of large numbers.
  Indeed, since the $X_n$ are assumed to be conditionally i.i.d.\ given $c_n$, and 
  ...
  Since this implies convergence in probability $\hat\theta \stackrel{P}{\to} \theta$,
  Lemma \ref{lem:g_derivatives_H0} gives that under $H_0$,
...
%  i.e.,
  ...
  Defining
  $$S_N := \sqrt{N} \diag\bigg(\frac{1}{\sqrt{\theta_{X|\ast}}} \otimes \sqrt{\hat\gamma}\bigg) (\hat\theta - \theta)$$
  where $\otimes$ denotes the Kronecker product of two vectors (in this case the all-ones vector $1_{\C{X}} \in \RN^{\C{X}}$ and the vector $\sqrt{\hat\gamma} \in \RN^{\C{C}}$),
  \Claude{parenthetical incorrectly describes the Kronecker vector as ``the all-ones vector $1_\mathcal{X}$''; the actual vector is $1/\sqrt{\theta_{X|\ast}}$}
  and the orthogonal projection\footnote{That is, $\hat\Gamma^T = \hat\Gamma = \hat\Gamma^2$.}
...
%    & = S_N^T \Big(I_{\C{X}} \otimes \big(I_{\C{C}} - \sqrt{\hat\gamma} \sqrt{\hat\gamma}^T\big)\Big) S_N \\
    & = S_N^T \hat \Gamma S_N 
...
%    & = \Big\lVert \Big(I_{\C{X}} \otimes \big(I_{\C{C}} - \sqrt{\hat\gamma} \sqrt{\hat\gamma}^T\big)\Big) S_N \Big\rVert^2.
    = \lVert \hat\Gamma S_N \rVert^2.
  \end{split}\end{equation*}
...
% \end{split}\end{equation*}
  Under $H_0$, $\theta_{x|c} = \theta_{x|\ast}$ for all $c \in \C{C}$, and therefore \eref{eq:multinomial_clt} simplifies to:
  \begin{equation*}
    \sqrt{N} \sqrt{\hat\gamma_c} (\hat\theta_{x|c} - \theta_{x|\ast}) \leadsto \C{N}(0,\Sigma)
  \end{equation*}
  where the covariance matrix $\Sigma$ from \eref{eq:multinomial_cov} simplifies to
  \begin{equation*}
    (\Sigma)_{x|c,x'|c'} = \delta_{cc'} \theta_{x|\ast} (\delta_{xx'} - \theta_{x'|\ast}).
  \end{equation*}
  Hence, scaling with $\sqrt{\theta_{x|\ast}}$ gives
  \begin{equation*}
    S_N = \sqrt{N} \sqrt{\hat\gamma_c} \frac{1}{\sqrt{\theta_{x|\ast}}} (\hat\theta_{x|c} - \theta_{x|\ast}) \leadsto \C{N}(0,\tilde{\Sigma})
  \end{equation*}
  where the covariance matrix $\Sigma$ from \eref{eq:multinomial_cov} simplifies to
  \begin{equation*}
    (\tilde{\Sigma})_{x|c,x'|c'} = \delta_{cc'} \Big(\delta_{xx'} - \sqrt{\theta_{x|\ast}} \sqrt{\theta_{x'|\ast}}\Big),
  \end{equation*}
...
%  and is seen to be an orthogonal projection on $\sqrt{\theta_{X|\ast}}$.
...
%  the covariance matrix $\Sigma$ given in \eref{eq:multinomial_cov} can be written as
%  $$(\Sigma)_{x|c,x'|c'} = \delta_{cc'} \theta_{x|c} (\delta_{xx'} - \theta_{x'|c}).$$
...
%  $$%\sqrt{N} \diag\bigg(\frac{1}{\sqrt{\theta_{X|\ast}}} \otimes \sqrt{\hat\gamma}\bigg) (\hat\theta - \theta) 
%    S_N \leadsto N(0,\tilde\Sigma),$$
%  with
%  $$\tilde\Sigma = \Big(I_{\C{X}} - \sqrt{\theta_{X|\ast}} \sqrt{\theta_{X|\ast}^T}\Big) \otimes I_{\C{C}}.$$
%  Note that $\tilde\Sigma$ is an orthogonal projection, hence $\tilde\Sigma = \tilde\Sigma^T \tilde\Sigma$.

  Let $\hat V \in SO(\RN^{\C{C}})$ be rotations that map $\sqrt{\hat\gamma}$ to $e_1$. Then
  $$I_{\C{C}} - \sqrt{\hat\gamma} \sqrt{\hat\gamma}^T = P_{\sqrt{\hat\gamma}^\perp} = \hat V^T P_{e_1^\perp} \hat V$$
  (where $P_{v^\perp}$ is the orthogonal projection on the subspace orthogonal to $v$),
  and therefore
  $$\lVert \hat\Gamma S_N \rVert^2 = \lVert (I_{\C{X}} \otimes \hat V^T P_{e_1^\perp} \hat V) S_N \rVert^2
    = \lVert (I_{\C{X}} \otimes P_{e_1^\perp})(I_{\C{X}} \otimes \hat V) S_N \rVert^2.$$
  We can apply Lemma~\ref{lem:weak_convergence_to_rotationally_symmetric_distribution} to conclude that $(I_{\C{X}} \otimes \hat V) S_N \leadsto \C{N}(0,\tilde\Sigma)$ as well (even though $\hat V$ is an $N$-dependent rotation).
  \Claude{Gap: the Lemma requires full rotational symmetry of the limiting distribution, but the actual limit $\mathcal{N}(0,\tilde\Sigma)$ has only partial rotational symmetry (invariant under $I_\mathcal{X}\otimes U$, not all rotations).  The argument is fixable by generalizing the lemma, but as written there is a mismatch.}
  Then 
  $$(I_{\C{X}} \otimes P_{e_1^\perp})(I_{\C{X}} \otimes \hat V) S_N \leadsto \C{N}\Big(0,P_{\sqrt{\theta_{X|\ast}}^\perp} \otimes P_{e_1^\perp}\Big)$$
  and hence
  $$\lVert \hat\Gamma S_N \rVert^2 \leadsto \chi^2_{\nu}$$
  with $\nu = (|\C{X}|-1)(|\C{C}|-1)$.
  With Slutsky's Lemma, also
  $$G_N = 2N g(\hat\theta,\hat\gamma) = N (\hat\theta - \theta)^T \nabla_\theta^2(\theta,\hat\gamma) (\hat\theta - \theta) + N o_P(\lVert\hat\theta - \theta\rVert^2) \leadsto \chi^2_{\nu}.$$
  \Claude{Gap: The Slutsky step $N\cdot o_P(\|\hat\theta-\theta\|^2)\to 0$ requires $N\|\hat\theta-\theta\|^2=O_P(1)$.  Under only $N_{+|c}\to\infty$ (not $N_{+|c}/N\ge\epsilon$), this is not guaranteed.}
\end{proof}
So, also the asymptotic distribution under the null hypothesis is the same as for the case of two random variables, 
...
\begin{Lem}\label{lem:weak_convergence_to_rotationally_symmetric_distribution}
  Let $Q$ be a rotationally symmetric probability measure on the standard Borel space $\RN^k$ (i.e., $Q \circ U = Q$ for all $U \in SO(\RN^k)$),
  and $P_1,P_2,\dots$ a sequence of probability measures on $\RN^k$. Then
  $$P_n \leadsto Q \iff P_n \circ U_n \leadsto Q$$
  for any sequence $U_1,U_2,\dots$ of rotations in $SO(\RN^k)$.
\end{Lem}
\begin{proof}
  We make use of the L\'evy-Prokhorov metrization of the weak topology. For two probability distributions $P,Q$ on
  $\RN^k$ (with its Borel $\sigma$-algebra $\C{B}_{\RN^k}$), 
  it is defined as
  $$d(P,Q) := \inf \{ \epsilon > 0 : \forall A \in \C{B}_{\RN^k} : P(A) \le Q(A^\epsilon) + \epsilon \ \land\ Q(A) \le P(A^\epsilon) + \epsilon \},$$
  where $A^\epsilon := \cup_{a \in A} B_\epsilon(a)$ with $B_\epsilon(a) := \{v \in \RN^k : \lVert v - a \rVert < \epsilon\}$.
  The L\'evy-Prokhorov metric is invariant under rotations. 
  Indeed, for $U \in SO(\RN^k)$, we have that $UA \in \C{B}_{\RN^k} \iff A \in \C{B}_{\RN^k}$, and $(UA)^\epsilon = U(A^\epsilon)$ for $A \in \C{B}_{\RN^k}$ and $\epsilon > 0$, hence $d(P\circ U, Q\circ U) = d(P,Q)$ for all probability measures $P,Q$ on $\RN^k$. 
  The rotation invariance of the L\'evy-Prokhorov metric implies that if $Q$ is rotationally symmetric (i.e., $Q = Q \circ U$ for all $U \in SO(\RN^k)$), then
  \begin{equation*}\begin{split}
    P_n \leadsto Q 
    &\iff d(P_n,Q) \to 0 \\
    &\iff d(P_n \circ U_n,Q \circ U_n) \to 0 \\
    &\iff d(P_n \circ U_n, Q) \to 0 \\
    &\iff P_n \circ U_n \leadsto Q
  \end{split}\end{equation*}
\end{proof}

Summarizing, we started from quite a different sampling scheme, and did not treat $C$ as a random variable, yet we
ended up with exactly the same likelihood ratio test for testing $X \Indep_{K(X|C)} C$ as we derived 
for testing the independence $X \Indep_{P(X,C)} C$ between two random variables.
...
Assume that $\theta \in \CX_\Theta$ is positive, i.e., such that $\theta_{x|c} > 0$ for all $x \in \C{X}, c \in \C{C}$.
Assume further that the fractions $N_{+|c} / N$ are bounded away from zero asymptotically,
i.e., there exists $\epsilon > 0$ such that for all $c \in \C{C}$, $N_{+|c} / N \ge \epsilon$ for large $N$.
...
In the proof of Proposition~\ref{prp:ext_G_test_asymptotics_H0}
we already saw that $\hat\theta \stackrel{a.s.}{\to} \theta$ for any $\theta \in \CX_\Theta$.
\Claude{Type~I: The stronger assumption $N_{+|c}/N\ge\epsilon$ closes the Slutsky gap from Proof~6, making the chi-squared approximation valid.}

Assume that $H_1$ holds, i.e., $\theta \in \CX_\Theta^1$.
Then $g(\theta,\gamma) = I(\gamma, \theta \circ \gamma) > 0$ for all $\gamma \in \CX_{\Theta_C}$.
$\CX_\Theta^1$ is open in $\CX_\Theta$, so $\bar B_{\delta}(\theta) \cap \CX_\Theta = \{\tilde{\theta} \in \CX_\Theta : \lVert \theta - \tilde{\theta} \rVert \le \delta \} \subseteq \CX_\Theta^1$ for $\delta$ small enough.
Since $\hat\theta \stackrel{a.s.}{\to} \theta$, $\hat\theta \in \bar B_{\delta}(\theta) \cap \CX_\Theta$ for large $N$ a.s..
By assumption, for large $N$ $\hat\gamma \in \{\tilde{\gamma} \in [\epsilon,1]^{|\C{C}|} : \tilde{\gamma}_+ = 1\}$,
which is a closed subset of $\CX_{\Theta_C}$.
Since $I$ is continuous, it attains a (positive) minimum value over the closed subset
$(\bar B_{\delta}(\theta) \cap \CX_\Theta) \times \{\tilde{\gamma} \in [\epsilon,1]^{|\C{C}|} : \tilde{\gamma}_+ = 1\} \subseteq \CX_\Theta \times \CX_{\Theta_C}$.
Hence, $G_N / N = 2g(\hat\theta,\hat\gamma)$ a.s.\ has a positive lower bound for large $N$.
We can now reason analogously as in the proof of Corollary~\ref{cor:G_test_consistency} to conclude that
...
%\Joris{But it seems we can weaken that assumption: if we don't want to use prior knowledge on which distributions $K(X \mid C=c)$ might differ, i.e., we need to sample from all values $c \in \C{C}$, it should suffice if $N_{+|c} \to \infty$, as in principle we could always throw away observations that occur too frequently asymptotically (resulting in $N_{x+c} / N \to 0$, for example). Perhaps we can choose the $\gamma_c$'s differently to reweight the data according to the value of $c$.}
...
%\PF{We could also use a split-likelihood-ratio test or an E-value approach. Might even be easier as won't need the asymptotics to $\chi^2$ and no regularity assumptions.}

\subsection{The general categorical case}

Finally, let us consider the most general case of testing a conditional independence involving transitional
random variables (including ``purely random'' and ``purely non-random'' variables as special cases).
Again, we will restrict ourselves to the case that all variables take values in finite spaces.
...
%\Joris{This was too general: take transitional random variables $X,Y,Z$ and kernel $K(X,Y,Z \mid C)$.
%This corresponds with taking $D = \emptyset$.}
Suppose we have three transitional random variables $X,Y,Z$ and an input variable $C$,
taking values in spaces $\C{X}, \C{Y}, \C{Z}, \C{C}$, respectively. 
Assume that all the spaces $\C{X},\C{Y},\C{Z},\C{C}$ are finite.
Suppose we have a kernel $K(X,Y,Z|C) : \C{C} \dshto \C{X} \times \C{Y} \times \C{Z}$.
We formulate a statistical test for testing the conditional independence
$$H_0 : X \Indep_{K(X,Y,Z|C)} Y \given Z$$
against the alternative
$$H_1 : X \nIndep_{K(X,Y,Z|C)} Y \given Z.$$
...
$$H_0 : X \Indep_{K(X,Y,Z|C)} Y,C \given Z$$
against the alternative
$$H_1 : X \nIndep_{K(X,Y,Z|C)} Y,C \given Z$$
instead.

Suppose we obtain data $(X_n,Y_n,Z_n,c_n)_{n=1}^N$ such that the
$(X_n,Y_n,Z_n)$ are conditionally independent and identically distributed given $c_n$ for all $n=1,\dots,N$.
In other words, we assume the data is sampled from the following Markov kernel:
$$K((X_n,Y_n,Z_n)_{n=1}^N \mid (c_n)_{n=1}^N) = \bigotimes_{n=1}^N K(X_n,Y_n,Z_n \mid C_n=c_n),$$
where each $K(X_n,Y_n,Z_n \mid C_n)$ is a copy of the (``true'' but unknown kernel) $K(X,Y,Z \mid C)$.
We parameterize this kernel $K(X,Y,Z \mid C)$ as
...
\end{equation}
With the correspondence $(X,(Y,C),Z) \leftrightarrow (X,Y,Z)$, this likelihood
ratio statistic is seen to be identical to \eref{eq:cond_G_test_statistic}, the one for the case
of three random variables. Note that this likelihood ratio treats $C$ and $Y$ on equal footing. 
This, again, suggests that for the asymptotic analysis we will obtain
a similar result as that for the conditional $G$ test with purely random variables, albeit under milder
assumptions on the sampling scheme for $C$. We will not work out the details here, but directly
formulate the result.

\begin{Prp}\label{prp:general_G_test_asymptotics_H0}
  Let $\theta \in \CX_\Theta$ be positive, i.e., such that $\theta_{xyz|c} > 0$ for all $x \in \C{X}, y \in \C{Y}, z \in \C{Z}, c \in \C{C}$.
  Assume that $N_{+++|c} \to \infty$ for all $c \in \C{C}$.
  Under $H_0 : X \Indep_{K(X,Y,Z \given C)} Y,C \given Z$, the likelihood ratio test statistic \eref{eq:general_G_test_statistic} converges
  to a $\chi^2$ distribution,
  $$G_N \leadsto \chi^2_\nu,$$
  with $\nu = |\C{Z}|\,(|\C{X}| - 1)(|\C{Y}||\C{C}| - 1)$ degrees of freedom.
\end{Prp}
\begin{proof}
  Note that the likelihood ratio test statistic \eref{eq:general_G_test_statistic} is a sum
  over $z \in \C{Z}$ of a likelihood ratio test statistic of the form 
  \eref{eq:ext_G_test_statistic} (where $(Y,C)$ in the former corresponds with $C$ in the latter). 
%  \Joris{Elaborate?}
  \Claude{Proof sketch with key steps omitted:
	\begin{itemize}
  \item Per-stratum assumptions of Proposition~\ref{prp:ext_G_test_asymptotics_H0} (positivity of all cell counts).
  \item Asymptotic independence of per-stratum statistics.
  \item Inherits the Slutsky remainder gap from Proposition~\ref{prp:ext_G_test_asymptotics_H0}.
	\end{itemize}}
\end{proof}
...
Consider an infinite sequence of $G$ tests performed on the first $N$ samples of an infinitely large
data set $(X_n,Y_n,Z_n,c_n)_{n=1}^\infty$,
where one decides
  $$\begin{cases}
  ...
  \end{cases}$$
for some given sequence of thresholds $\tau_N$.
Assume that $\theta \in \CX_\Theta$ is positive, i.e., such that $\theta_{xyz|c} > 0$ for all $x \in \C{X}, y \in \C{Y}, z \in \C{Z}, c \in \C{C}$.
...
one can find a general analysis in \cite{LomnickiZaremba1959} (for arbitrary $\theta_{xy} \in \CX_\Theta$). 
...
Under $H_1 : X \nIndep Y$ (equivalently, if $I := I(X;Y) > 0$), and with regularity assumption \eref{eq:G_test_regularity},
  $$\sqrt{N}(\hat I_N - I) \leadsto \C{N}(0,\sigma^2)$$
where
...
%  \end{split}\end{equation*}
  ...
  $$\sqrt{N}(\hat\theta - \theta) \leadsto \C{N}(0,\Sigma)$$
...
%  regularity conditions.
  ...
  $$\sqrt{N}\frac{\hat\theta - \theta}{\sqrt{\theta}} \leadsto \C{N}(0,\tilde\Sigma)$$
  with
  $$\tilde\Sigma = \frac{1}{\sqrt{\theta}} \Sigma \frac{1}{\sqrt{\theta}}^T = I - \sqrt{\theta} \sqrt{\theta}^T.$$
   ...
   $$\sqrt{N} \tilde{\Sigma} \frac{\hat\theta - \theta}{\sqrt{\theta}} \leadsto \C{N}(0,\tilde\Sigma)$$
...
%\section{Conditional Independence Testing?}
%
%\PF{Do we want to do some more conditional independence testing this year in the lecture?}
