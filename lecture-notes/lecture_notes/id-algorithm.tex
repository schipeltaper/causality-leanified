...
\subsection{The ID-Algorithm}
        ...
        For disjoint subsets $A,B,C \ins V$ we want to infer the conditional interventional distribution $P(X_A|X_B,\doit(X_{J \cup C}))$ 
...
\begin{Def}[Identifiability of interventional distributions/Markov kernels] 
    ...
    if for every two L-CBNs $M_1$ and $M_2$ with the same:
    \begin{enumerate}
...
\begin{Def}[Trackability (up to specifications)]
    ...
    indicating on which variable the operation is applied to, such that for every L-CBNs $M$ with \observational CADMG $G$ 
...
\begin{Eg}
    ...
    We assume that the observational distribution $P(X_1,X_2,X_3)$ is given.
...
\begin{Not}[The key interventional Markov kernels]
     Let $M$ be an L-CBN with with \observational CADMG $G=(J,V,E,L)$ and $C \ins V$ any subset.
     We will abbreviate:
    \begin{align*} \Qf[C] 
...
\begin{Lem}
    ...
    \begin{proof}
        ...
        \[ C \idPerp_{G_{\doit(I_{V \sm (C \cup \Pa^G(C))},\Pa^G(C)\sm C)}} I_{V \sm (C \cup \Pa^G(C))} \given \Pa^G(C)\sm C.\]
...
\begin{Prp}[Interventional ordered local Markov property]
    \label{prp:ioLMP}
    Let $M$ be an L-CBN with with \observational CADMG $G=(J,V,E,L)$ and a fixed topological order $<$.
    ...
    \begin{proof}
        This follows from the global Markov property, Theorem \ref{thm-gmp-mCBN}, together with the $id$-separation statement:
        \[ \lC v\rC \idPerp_{ G_{\doit(I_{V \sm \Di^G_\le(v)})}}  \Pred^G_<(v) \given \MBl^G_<(v).\]
...
\begin{Lem}
   ...
   Then we have the $id$-separation statement:
   \[ \lC v \rC \idPerp_{G_{\doit(I_{V\sm D_\le})}}  V_< \given \Pa^G(D_\le) \cup D_<.  \]
   %\[ \lC v \rC \idPerp_{G_{\doit(I_{V\sm D_\le})}} I_{V\sm D_\le} \dcup J \dcup V_< \given \Pa^G(D_\le) \cup D_<.  \]
   \begin{proof}
       We abbreviate:
       \[\tilde G := G_{\doit(I_{V\sm D_\le})}, \qquad F:= \Pa^G(D_\le) \cup D_<.\]
       Assume the contrary to the claim and let $\pi$ be a shortest path from $v$ to a node $w \in J \dcup I_{V \sm D_\le} \dcup V_<$ \Claude{$I_{V \sm D}$ should be $I_{V \sm D_{\le}}$}\PF{solved} in the graph $\tilde G$. It is clear that $w \neq v$.
       ...
       But this is a contradiction to $w \in J \dcup I_{V \sm D_\le} \dcup V_<$\Claude{$I_{V \sm D}$ should be $I_{V \sm D_{\le}}$}\PF{solved}. So this case cannot occur.
...
\begin{Lem}[Interventional ordered local Markov property]
    ...
    Let $M$ be an L-CBN with with \observational CADMG $G=(J,V,E,L)$ and a fixed topological order $<$ and fixed $v \in V$.
 ...
 \begin{proof}
        By Lemma \ref{lem:id-d-sep-3} we have the $id$-separation:
   \[ \lC v \rC \idPerp_{G_{\doit(I_{V\sm D_\le})}} V_< \given F.  \]
   By the global Markov property, Theorem \ref{thm-gmp-mCBN}, we get: 
   \[ X_v \Indep_{P(X_{V_\le}|\doit(X_{I_{V\sm D_\le}},X_J))}  X_{V_<} \given X_F.\]
...
\begin{Lem}[Ancestral subsets are \trackable]
    \label{lem:int-anc}
    Let $M$ be an L-CBN with with \observational CADMG $G=(J,V,E,L)$ and $A \ins V$ be a subset such that 
    $A=\Anc^{[V]}(A)$.
    ...
    \begin{proof}
       ...
       The latter follows again from the global Markov property and the $id$-separation:
       \[ A \idPerp_{G_{\doit(I_{V \sm A})}} I_{V \sm A} \given J.  \]
      % The arguments are similar to \ref{lem:C-intPaC} with the  $\Pa^G(A) \sm A \ins J$.
       This $id$-separation holds true since every walk from a node $v \in A$ to a node $j \in J \dcup I_{V \sm A}$ is either blocked by $J$ as the endnode $j \in J$ or is of the form:
...
\begin{Rem}[Districts are trackable up to oracle choices] 
    ...
    Let $M$ be an L-CBN with \observational CADMG $G=(J,V,E,L)$.
    \begin{enumerate}
...
\begin{Def}
    \label{not:Q-dist}
        Let $M$ be an L-CBN with \observational CADMG $G=(J,V,E,L)$ and a fixed topological order $<$.
...
\begin{Thm}[Soundness up to null-sets]
    ...
    Consider the class of L-CBNs $M$ with \observational CADMG $G$ such that the following holds: 
    \begin{enumerate}
...
\begin{Rem}
    ...
    \begin{enumerate}
        ...
        \item The conditions of Theorem \ref{thm:ID-as-soundness} are satisfied for a L-CBNs $M$ with CDAG
...
\begin{Eg}
    ...
    are satisfied if every Markov kernel of the L-CBN $M$ is linear Gaussian: 
...
\begin{Thm}[Completeness, see \cite{HV08s}] 
    ...
    then there exist two L-CBNs $M_1$ and $M_2$ with the same \observational CADMG
...
\begin{Thm}[Soundness up to null-sets]
    ...
    Now let:
       \[ \tilde N := \lC x_{J \cup V \sm A_\ge} \in \Xcal_{J \cup V \sm A_\ge}  
            \st K(X_v|X_{J \cup V \sm A_\ge}=x_{J \cup V \sm A_\ge})
        \neq Q(X_v|X_{\MBl^{G'}_<(v)} = x_{\MBl^{G'}_<(v)}) \rC, \]
        \Claude{$\MBl^G_<(v)$ should be $\MBl^{G'}_<(v)$}\PF{solved.}
        and $N:= \tilde N \times \Xcal_{A_\ge} \ins \Xcal_{J \cup V}$. 
       ...
       & = K(X_{v_K}|X_{J \cup V \sm A_{\ge v_K}}) \otimes Q(X_{v_K}|X_{\MBl^{G'}_<(v_K)}) \Claude{\text{should be:} K(X_{v_{K-1}}|X_{\MBl^{G'}_<(v_K)})}\PF{\text{disagree with Claude here.}} \otimes P(X_{A_{< v_{K-1}}}|\doit(X_{J \cup V \sm A})) \otimes \mu_{V \sm A}(X_{V \sm A}) \\
           & = \cdots \\
...
\begin{Thm}[Soundness up to continuous choices for strictly positive CBNs]
    ...
    \begin{proof}
     ...
     \Joris{TODO: let Claude work out the details for the recursion.}
    \end{proof}
\end{Thm}
...
\begin{Lem}
    ...
    \bigotimes_{ u \in U}^> P_u (X_u \in dx_u|\doit( X_{\Pa^{G^+}(u)} = x_{\Pa^{G^+}(u)} )).   \]
    \Claude{$\Pa^{G^+}(v)$ should be $\Pa^{G^+}(u)$}\PF{solved.}
