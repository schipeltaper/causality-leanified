...
\begin{proof}
Because $id$-separation is preserved under marginalization:
\[ A \idPerp_G B \given C \quad \iff \quad A \idPerp_{G^+} B \given C,   \]
...
%where the product goes in reverse order of $<$. We will write $\iPerp$ for $\idPerp_G$ and $\Indep$ for 
%$\Indep_{P(X_V|\doit(X_J))}$ in the following.\\

0.) Induction start:  $V= \emptyset$. This means that $A,B,C \ins J$.
The assumption: 
\[A \idPerp_G  B \given C,\] 
implies that we must have that $A \ins C$. Otherwise a trivial walk from $A \ins J$ to $J \cup B$ would be $C$-$d$-open.
...
(with input variables, but without latent variables and without bidirected edges) 
with $\#V < n$ (and arbitrary $J$).\\

1.) Now assume: $\#V =n>0$ and $A \idPerp_G B \given C$.
...
Note that, since $\Ch^G(v) = \emptyset$, the marginalization $G^{\sm \{v\}}$ has no bidirected edges and thus induces again 
a causal Bayesian network without latent variables with $\#V^{\sm \{v\}}=n-1 <n$.\\
Furthermore, we have the factorization:
\begin{align*} 
    P(X_V|\doit(X_J)) 
        &= P_v(X_v|X_{\Pa^G(v)}) \otimes 
    \underbrace{\bigotimes_{w \in \Pred^G_<(v) \sm J} P_w(X_w|X_{\Pa^G(w)})}_{P(X_{\Pred_<^G(v) \sm J}|\doit(X_J))}.
\end{align*}
...
implies that:
\[ A \cap ( J \cup B) \ins C.  \]
Otherwise a trivial walk from $A$ to $J \cup B$ would be $C$-$d$-open. 
...
\[A \idPerp_{G}  B \given C\quad \iff \quad A \idPerp_{G^{\sm v}}  B \given C.\]
...
\begin{align*}
    A \idPerp_G  B \given C & \xRightarrow{\text{Right Decomposition}}  & A \idPerp_G B' \given C \\
    & \xRightarrow{\text{Left Decomposition}}  & A' \idPerp_G  B' \given C \\
    & \xRightarrow{\text{marginalization, } v \notin A'\cup J \cup B' \cup C}  & A' \idPerp_{G^{\sm \{v\}}}  B' \given C \\
...
\begin{align*}
 A \idPerp_G  B \given C & \xRightarrow{\text{Right Decomposition, }B' \ins B}  & A \idPerp_G   B' \given C   \\
  & \xRightarrow{\text{Left Weak Union, } A= A' \dcup (A \cap C) \dcup \{v\}}  & \{v\} \idPerp_G  B' \given A' \dcup C    \\
 & \xRightarrow{(*), \text{ see below}}  & D  \idPerp_G   B' \given A' \dcup C \\ 
 & \xRightarrow{\text{marginalization, } v \notin D \cup J \cup B' \cup A' \cup C}  & D \idPerp_{G^{\sm \{v\}}}  B' \given A' \dcup C \\
...
\end{align*}
(*) holds since every $(A' \dcup C)$-$d$-open walk $ w \sus \cdots$ from a  $w \in D=\Pa^G(v)$ to $J \cup B'$ 
extends to an $(A' \dcup C)$-$d$-open walk from $v$ to $J \cup B'$ via  $v \hut w \sus \cdots$, as $w$ stays a 
...
\end{align*}
\Joris{The $X_J$-Inverted Right Decomposition step actually combines several steps. I'm not sure if this one follows?}
...
A  \idPerp_G B \given C 
& \xRightarrow{\text{Left Decomposition}}  & A' \idPerp_G  B \given C \\
& \xRightarrow{\text{Right Decomposition}}  & A' \idPerp_G  B' \given C \\
 & \xRightarrow{\text{marginalization, } v \notin A'\cup J \cup B' \cup C}  & A' \idPerp_{G^{\sm \{v\}}}  B' \given C \\
 & \xRightarrow{\text{induction (IND)}}  & X_{A'} \Indep_{P(X_V|\doit(X_J))}  X_{B'} \given X_C. \label{eqn:bli'} \tag{\#1'}
\end{align*}
Again with $D = \Pa^G(v)$ we get:
\begin{align*}
A  \idPerp_G  B \given C  & \xRightarrow{\text{Left Decomposition}}  & A' \idPerp_G  B \given C \\
 & \xRightarrow{\text{Right Decomposition}}  & A' \idPerp_G  B' \cup \{v\} \given C \\
& \xRightarrow{\text{Right Weak Union}}  & A' \idPerp_G  \{v\} \given B' \dcup C  \\
 & \xRightarrow{(\bullet), \text{ see below}}  & A'  \idPerp_G  D \given B' \dcup C \\  
  & \xRightarrow{\text{marginalization, } v \notin A' \cup J \cup D \cup B' \cup C}  & A'  \idPerp_{G^{\sm \{v\}}}  D \given B' \dcup C \\  
...
\end{align*}
$(\bullet)$ holds since every $(B' \dcup C)$-$d$-open walk $\cdots \sus w$ from $A'$ to a $w \in J \cup D$ extends to 
a $(B' \dcup C)$-$d$-open walk from $A'$ to $J \cup \{v\}$, either because $w \in J$ or via $\cdots \sus w \tuh v$ if 
$w \in D=\Pa^G(v)$.
...
& \xRightarrow{\text{Left Decomposition}}  & A' \idPerp_G  B \given C \\
& \xRightarrow{\text{Right Decomposition}}  & A' \idPerp_G  B' \given C \\
& \xRightarrow{C = C' \dcup \{v\}}  & A' \idPerp_G  B' \given C'\dcup\{v\} \\
\end{align*}
...
So there exist shortest $C'$-$d$-open walks $\pi_1$ and $\pi_2$ in $G$ such that all colliders are in $C'$:
...
If now $\pi_i$ does not contain $v$ as an (end) node, then $\pi_i$ would be $(C'\dcup \{v\})$-$d$-open,
...
is a $(C' \dcup \{v\})$-$d$-open walk from $A'$ to $J \cup B'$, in contradiction to:
\[A' \idPerp_G B' \given C'\dcup\{v\}.\]
So the claim:
\[  A' \dcup \{v\} \idPerp_G  B' \given C' \qquad \lor \qquad A' \idPerp_G B' \dcup \{v\} \given C', \]
must be true. 
