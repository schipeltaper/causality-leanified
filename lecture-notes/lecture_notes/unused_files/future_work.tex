...
\begin{Def}\label{def:cdg_sparse_acyclification}
  ...
  \end{enumerate}
\end{Def}
The notion of $i\sigma$-separation no longer works, but:
\begin{itemize}
  \item One can directly apply $id$-separation to the sparse acyclification.
  \item One can exploit the deterministic relations by applying Geiger's D-separation to the sparse acyclification instead of $id$-separation.
...
Note that by starting from the bipartite graphical representation and then applying causal ordering, and then applying D-separation to the ordered (acyclified) graph after marginalizing out the factor nodes, we get an even stronger Markov property than can be obtained from SCMs.
  ...
  This leads to additional independences that are not entailed by $i\sigma$-separation.
  ...
  \begin{cases}
    ...
    X_2 = g_2(X_3,X_a,X_b)
  \end{cases}$$
  Thus we get all independences implied by $i\sigma$-separation on \emph{any} of the following three graphs:
  \begin{center}
  \begin{tikzpicture}
...
\begin{Prp}
  ...
  Define 
  $E^+ := \{j \tuh v : j \in J, v \in V, \exists \text{ walk } j \sus j_1 \sus \dots \sus j_n \tuh v \text{ in } \bar{G} \text{ with } j_1,\dots,j_n \in J \text{ and each } j_i \text{ a collider or an unblockable non-collider}\}$
  ($n$ could be 0, i.e., $j \suh v$ also leads to an edge $j \tuh v$), and
  $L^+ := \{v \huh w : v \ne w \in V, \exists \text{ walk } v \hus j_1 \sus \dots \sus j_n \suh w \text{ in } \bar{G} \text{ with } j_1,\dots,j_n \in J \text{ and each } j_i \text{ a collider or an unblockable non-collider}\}$ ($n$ could be 0, i.e., $j \suh v$ also leads to an edge $j \tuh v$).
...
\end{proof}
\begin{Con}
  If we can generalize this result to start with a CDMG rather than a DGM, then we also get a do-calculus.
...
$$\exists Q : X \Indep^w Y,T_1 | Z,T_2 \impliedby X \iPerp Y,T_1 | Z,T_2$$
The faithfulness assumptions we want to make is:
$$\forall Q : X \Indep^w Y | Z,T \implies X \iPerp Y | Z,T$$
(Wait, isn't that too weak??? Don't we want
$$\forall Q : X \Indep^w Y,T_1 | Z,T_2 \implies X \iPerp Y,T_1 | Z,T_2$$
instead?)
If we can replace this by:
$$\exists Q > 0 : X \Indep^w Y | Z,T \implies X \iPerp Y | Z,T$$
...
What we would need is a Markov property that shows:
$$(\forall Q(T) : X \Indep_{K(W|T)\otimes Q(T)} Y,T_1 | Z,T_2) \impliedby X \iPerp Y,T_1 | Z,T_2,$$
...
The faithfulness assumptions we want to make is:
$$\forall Q(T) : X \Indep_{K(W|T)\otimes Q(T)} Y | Z,T \implies X \iPerp Y | Z,T$$
If we can replace this by:
$$\forall Q(T) > 0 : (X \Indep_{K(W|T)\otimes Q(T)} Y | Z,T \implies X \iPerp Y | Z,T)$$
then we can test everything with probabilistic CI tests!
...
\begin{Prp}\label{prp:sigma_sep_as_s_sep}
  ...
  $$A \sPerp_G B \given C \iff A \Perp^s_G B \given C \iff A \Perp^s_{H(G)} B \given C.$$
\end{Prp}
...
\centerline{\begin{tikzpicture}
  \begin{scope}
    \node[ndout] (i) at (0,0) {$i$};
    \node[ndout] (v) at (1.5,0) {$v$};
    \node[ndout] (w) at (3,0) {$w$};
    \node[ndout] (j) at (4.5,0) {$j$};
    \draw[arout] (i) to (v);
...
In such a way that it corresponds one-to-one to $i\sigma$-separation in a directed version of the bipartite graph according to a perfect matching? 
