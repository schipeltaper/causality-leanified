...
\end{align*}
    ...
    \[ X = R(E|Z) \quad K(U,X|Z)\text{-a.s.}\]
    \begin{proof}
        Since $F(x;0|z)$, $F(x;1|z)$ and $R(e|z)$ are monotone and one-sided continuous in the first argument and measurable in the second argument $z$, they are jointly measurable (as functions of two arguments).
        This directly implies the joint measurabilities of $F$ and $R$. 
        The statement then directly follows from Lemma \ref{unif-cdf} by applying it for every $z$ separately.
        \Claude{Joint measurability of $F(x; u|z)$ and $R(e|z)$ in all variables is explicitly deferred and non-trivial (it requires measurability of $(x, z) \mapsto K(X < x|Z = z)$ and of the infimum operation $(e, z) \mapsto R(e|z)$). TODO: flesh out details.}\PF{I added a few more arguments, but didn't prove the claims.}
    \end{proof}
\end{Thm}
