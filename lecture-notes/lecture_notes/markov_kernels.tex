...
\subsection{Finite Transition Measures and Markov Kernels}
...
\begin{Def}[Finite transition measures and Markov kernels]
    \label{def-finite-transition-measure}
    Let $\Tcal$, $\Wcal$ be measurable spaces.
    \begin{enumerate}
      \item A \emph{(finite\footnote{In this course we will only discuss \emph{finite} transition measures.}) transition measure}  from $\Tcal$ to $\Wcal$
...
\begin{Def}[Absolute continuity]\label{def:absolute_continuity}
...
\begin{Def}[Null sets]\label{def:finite-transition-measure-nullsets}
...
\begin{proof}
    ...
    \[ Q(W) \ll \mu \ll Q(W). \]
    Indeed, if $\mu$ is finite, we can just put $Q(W \in D) := \frac{\mu(D)}{\mu(\Wcal)}$. If $\mu$ is $\sigma$-finite, but not finite, then we have a disjoint decomposition $\Wcal = \bigdcup_{n \in \N} \Wcal_n$ with $0<\mu(\Wcal_n)<\infty$. We can then put:
    \[
        Q(W \in D) := \sum_{n \in \N} 2^{-n} \frac{\mu(D \cap \Wcal_n)}{\mu(\Wcal_n)},
    \]
    where here $\N:=\lC 1,2, \dots \rC$. 
    \Claude{Is $\N=1,2,\dots$ or $\N=0,1,2,\dots$?} \PF{solved.}
...
\begin{Cor}[Absolute continuity and strictly positive densities]
    ...
    Let $\Tcal$, $\Wcal$ be measurable spaces, where $\Wcal$ is standard\footref{fn:count-generated}, and:
...
\begin{Thm}[Existence of the Doob-Radon-Nikodym derivative, see \cite{Del83} Thm.\ 58, \cite{Kle20} Ex.\ 11.17]
 ...
 Let $m \ge n$ then we get:
     ...
     \I_{K(W \in D|T=t)>0} \rB  \cdot \I_D(w)  \\
             ...
             \underbrace{ \lB \sum_{\substack{D \in \Ecal_n\\ D \sni A}}  \frac{K(W \in A|T=t)}{K(W \in D|T=t)}\cdot  \I_{K(W \in D|T=t)>0}   \cdot \I_D(w)\rB }_{=\E_t[\I_A|\Bcal_n](w)}  \\
    ...
    $D \in \Bcal_\Wcal=\sigma\lp \bigcup_{n \in \N} \Ecal_n\rp$ via Dynkin's lemma\Claude{ via Dynkin's $\pi$-$\lambda$ theorem}\PF{solved.}, and we get:
    \begin{align*}
...
\begin{comment}
\begin{Def}[Composition of Markov kernels]%\label{def-composition-markov-kernels}
    Consider two Markov kernels:
