...
\begin{Rem}[Recap]
    ...
    Further, for $D \ins J \cup V$ and $B \ins V \sm D$ we get the combined hard and soft interventions:
    \[ P\lp X_{V \sm D},X_U,X_{J \cup D},X_{I_B}| \doit(X_{J \cup I_B \cup D}) \rp :=\] 
    \[    \bigotimes_{v \in V \sm (B \cup D)} P_v\lp X_v|\doit(X_{\Pa^{G^+}(v)})\rp \otimes 
    \bigotimes_{v \in B} P_v\lp X_v|\doit(X_{\{I_v\}\cup\Pa^{G^+}(v)})\rp \otimes \]
     \[   \bigotimes_{v \in U} P_v\lp X_v|\doit(X_{\Pa^{G^+}(v)})\rp \otimes 
     \bigotimes_{j \in J \cup D} \delta(X_j|X_j) \otimes\bigotimes_{v \in B} \delta(X_{I_v}|X_{I_v})  ,\]
       where we need to reorder all the factors such that the product is in reverse order of a topological order and 
       where we use the following Markov kernels to model hard interventions as  soft interventions, $v\in B$: 
  ...
  \[ P\lp X_{A\cup B}|X_C, \doit(X_{J \cup I_B \cup D}) \rp ,\]
  for any $A,B,C \ins V$, $D \ins V \cup J$ with $A,B,C,D$ disjoint (w.l.o.g.\ we can assume $A \ins V$, $B \cap C = \emptyset$ and $B \cap D = \emptyset$).
  %for any $A,B,C,D \ins J \cup V$ \Joris{Really ``any''? I thought you assumed $B \ins V \sm D$? With the new convention on $\doit(I_J)$ the restrictions have become redundant; however Patrick noted that the way of writing things might matter: rather than $\doit(X_{I_B},X_{J \cup D})$ one should write $\doit(X_{J \cup D \cup I_B})$.\\
For more suggestive formulas later on we also freely permute the order of symbols behind the conditioning line, e.g. (if $F \cap D = \emptyset$):
\[ P\lp X_A|\doit(X_F),X_C, \doit(X_D) \rp := P\lp X_A|X_C, \doit(X_D,X_F) \rp := P\lp X_A|X_C, \doit(X_{D\cup F}) \rp.\]
...
We will also make use of the following CADMG (for $I_B \cap D = \emptyset$):
  \[ G_{\doit(I_B,D)} := G_{\doit(I_B\cup D)} = (G^+_{\doit(I_B \cup D)})^{\sm U}.  \]
%W.l.o.g.\ we can assume: $B \cap D = \emptyset$. \Joris{This is mysterious: explain why, or remove assumption.}
\end{Rem}
...
\end{proof}
\begin{Rem}
  We have made the following considerations in this particular formulation of the do-calculus.
  \begin{itemize}
    \item The additional assumptions $J \ins D$ in rule 1--3 were added because it is not clear whether (or how) a Markov kernel $P(X_A|X_C,\doit(X_{D}))$ is defined for $J \not\ins D$. 
      Rule 4 was added to make this possible.
    \item The assumption $J \subseteq D$ in rules 1--3 in Corollary~\ref{cor:as-do-calculus-2} can be made without loss of generality and without restricting the applicability of the result. For example, for rule 1, if $\tilde D$ with $J \not\ins \tilde D$, then $A \idPerp_{G_{\doit(\tilde D)}} B | C \cup \tilde D$ implies $A \idPerp_{G_{\doit(\tilde D)}} B \cup J | C \cup \tilde D$, which implies $A \idPerp_{G_{\doit(\tilde D \cup J)}} B | C \cup (\tilde D \cup J)$, and hence we can apply rule 1 to $D = \tilde D \cup J$ instead of $\tilde D$.
      Similar reasoning can be applied to show for rules 2 and 3 that the assumptions $J \ins D$ can be made without loss of generality.
    \item \Joris{Furthermore, one may assume $D = J$ in rules 1--3 and $D \ins J$ in rule 4. This can be done without loss of generality,
      because one can always apply that special case to the intervened CBN $M_{\doit(\tilde{D})}$ for $\tilde{D} = D \sm J$ to obtain the general case.
      In this way the rules become shorter and easier to remember.}
    \item \Joris{Yet another possible formulation would be obtained by dropping the assumptions $J \ins D$ in rules 1--3, and replacing all occurrences of $D$ by $D \cup J$ in the various Markov kernels occurring in these rules.}
  \end{itemize}
\end{Rem}


\begin{Rem}
Note that in rules 1--3 of Corollary \ref{cor:as-do-calculus-2} (in contrast to Proposition \ref{prp:do-calculus} and Theorem \ref{thm:as-do-calculus-1})  we cannot easily formulate the independence of variables $X_{J \sm D}$ in the presented way. 
 This is the reason we decided to simplify the formulas in Corollary \ref{cor:as-do-calculus-2} by assuming that $J \ins D$ and thus $D \cup J = D$, which makes then the $id$-separation requirements weaker (due to extra conditioning on $J$). 
 To compensate for the ensuing loss of generality, we have added the fourth rule.
For the case where $J$ does not fully lie in $D$ one has several options:
  \begin{enumerate}
    \item use Proposition~\ref{prp:do-calculus} or Theorem~\ref{thm:as-do-calculus-1}, or 
    \item use the global Markov property (Theorem~\ref{thm-gmp-mCBN}) directly, or
    \item combine rules 1--3 of Corollary~\ref{cor:as-do-calculus-2} with rule 4 of Corollary~\ref{cor:as-do-calculus-2}.
  \Joris{TODO: Patrick and I agreed to replace this remark by one where we show how rule 1,2,3 can be combined by rule 4 to prove a stronger statement under stronger conditions.}
  \end{enumerate}
\end{Rem}
