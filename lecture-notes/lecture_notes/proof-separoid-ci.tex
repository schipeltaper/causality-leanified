\paragraph{Proofs - Separoid Axioms for Conditional Independence}\hfill%
...
\begin{proof}
    If $U=\varphi(X)$ put
    $Q(U \in D|X=x):=\delta_\varphi(U \in D|X=x):=\I_D(\varphi(x))$ for $D \subseteq \Ucal$. Then we get:
%\begin{align*} 
%    \E_t\lB \delta_\varphi(U \in D|X_t) \cdot \I_B(Y_t) \cdot \I_C(X_t) \rB &= \E_t\lB \I_D(U_t) \cdot \I_B(Y_t) \cdot \I_C(X_t) \rB .     
%\end{align*}
\begin{align*}
&\int_C \int_B  Q(U \in D|X=x)\, K(Y \in dy, X \in dx|T=t) \\
&= \int_C \int_B  \delta_\varphi(U \in D|X=x)\, K(Y \in dy, X \in dx|T=t) \\
&= \int_C \int_B  \I_{\varphi^{-1}(D)}(x) \, K(Y \in dy, X \in dx|T=t) \\
&= K(Y \in B, X \in C \cap \varphi^{-1}(D)|T=t) \\
&= K(Y \in B, X \in C, \varphi(X) \in D |T=t)\\
...
\begin{proof}
    ...
    \[ K(X,Z|T) = K(X|Z,T) \otimes K(Z|T),\]
     with the conditional Markov kernel $K(X|Z,T)$ of $K(X,Z|T)$ (via theorem \ref{thm-regular-conditional-Markov-kernel}). 
    Introducting $\ast$ and $T$ to both sides gives the factorization:
    \[ K(X,\ast, Z,T|T) = K(X|Z,T) \otimes K(\ast, Z,T|T).\]
    \Claude{The conditioning variable is $(Z,T)$, so $\Zcal\times\Tcal$ plays the role of $\Ycal$ in the disintegration theorem.  The lemma only assumes $\Xcal,\Zcal$ standard, not~$\Tcal$.  The proof is correct if the disintegration is applied \emph{pointwise in~$t$} (for each $t$, disintegrate $K(X,Z|T=t)$, which only needs $\Xcal,\Zcal$ standard).  This pointwise reading is not made explicit.}\PF{solved by introducing one further step on how the theorem is applied first. Claude's comment is rather misleading.}
...
\begin{proof}
    ...
    This shows the claim using the same $Q(X|Z)$. 
    In suggestive symbols:
...
\begin{proof}
    Let $Q(X,U|Z)$ be given from the conditional independence assumption.
    Then we have:
...
\begin{proof}
    Let $Q(X|Z)$ be given from the conditional independence assumption. 
    We then have:
...
\begin{proof}
    ...
    \end{align}
    If we marginalize out $U$ in equation \ref{eq:rcc-e} and use equation \ref{eq:rcc-c2}  we arrive at:
    \begin{align}
...
Plugging equation \ref{eq:ess-un} back into the equation \ref{eq:rcc-a} we get:
    \begin{align}
        K(X,Y,U,Z|T) & = Q(X|U,Z) \otimes K(Y,U,Z|T),\\
...
In suggestive symbols:
\[ K(Y|\cancel{\ast,T,X},Z) = K(Y|\cancel{T},Z,\ast).\]
\end{proof}
\end{Lem}
