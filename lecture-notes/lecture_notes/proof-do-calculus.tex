...
\begin{Lem}\label{lem:int-null-set}
    For $B,C \ins V$ and $D \ins V \cup J$ with $B,C,D$ pairwise disjoint, and a measurable subset $N \ins \Xcal_{B \cup C\cup D \cup J}$ the following statements are equivalent:
    \begin{enumerate}
...
\begin{Rem}[Null sets---again]\label{rem:null-sets}
...
%    \Joris{In rule 3, should we allow for $B \subseteq V \cup J$? Or: does rule 4 reduce to an existing rule now that we identify $I_j$ with $j$ for $j \in J$? Patrick pointed out that part of the statement is about the existence of a certain Markov kernel, and this aspect of rule 4 may NOT reduce to a special case of an (extended) rule 3. One should also be very careful when dropping the assumption $B \subseteq V$ because it is used \emph{extensively} throughout all the proofs. It seems that rule 4 can more easily be obtained as the special case $B=I_B=\emptyset$ rather than $B \subseteq V \cup J$.}
    ...
    \begin{enumerate}
        ...
        \item[] Such a Markov kernel is unique up to a measurable $P(X_B,X_C|\doit(X_{I_B},X_{D\cup J}))$-null set $N \ins \Xcal_{B \cup C\cup D}$, i.e.\ $N$ is a $P(X_{B_1},X_C|\doit(X_{B_2},X_{D\cup J}))$-null set
            for every decomposition $B=B_1\dcup B_2$ simultaneously.
...
We first claim that for a Markov kernel $Q(X_A|X_B,X_C,X_D)$ the equation \ref{eq:2nd-do-int-1} is equivalent to the system of equations \ref{eq:2nd-do-int-2} indexed by subsets $B_2 \ins B$ and with $B_1 := B \sm B_2$:
\begin{align}
...
\end{align*}
If $K(X_A|X_B,X_C,X_D)$ now is also a version of the conditional Markov kernel \eref{eq:2nd-do-int-4} 
