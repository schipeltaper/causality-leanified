...
\begin{tikzpicture}[scale=1, transform shape]
    ...
    \node[ndout] (T) at (3,1) {$T$};
    \node[ndout] (U) at (3,-1) {$U$};
    \node[ndlat] (R) at (3,3) {$R$};
...
\begin{enumerate}[label=\alph*)]
    \item $P(T,U|\doit(X,W)) = P(T,U|X,\doit(W))$. 
      \score{2}
    \item $P(Y|T,U,\doit(X,W)) = P(Y|\doit(X,T,U,W))$.
      \score{2}
    \item $P(Y|\doit(X,T,U,W)) = P(Y|\doit(T,U,W))$.
      \score{2}
    \item $P(X|\doit(T,U,W)) = P(X|\doit(W))$.
      \score{2}
    \item $P(Y|X,\doit(T,U,W)) = P(Y|X,T,U,\doit(W))$.
      \score{2}
\end{enumerate}
...
\begin{enumerate}[resume*]
  \item Derive an expression for the Markov kernel $P(Y|\doit(X,W))$ in terms of (products and compositions of
    marginals and conditionals of) $P(X,Y,T,U|\doit(W))$.
      \score{1}
...
\end{align}
for suitably chosen variables $Z$.
}
\end{enumerate}
...
First note that in all expression $T,U$ appear together and we can thus gather and abbreviate them as the one variable: $Z=(T,U)$.
Furthermore, since all occurring Markov kernels contain $\doit(W)$ and $L,R$ do not appear we can intervene on $W$ and marginalize out $L,R$ and get the following CADMG 
$\tilde G = G_{\doit(W)}^{\sm (L,R)}$:
...
\[ Z \Perp^d_{G_{\doit(I_X,W)}^{\sm(L,R )}} I_X \given X,W,\]
in the CADMG:
\centerline{
\begin{tikzpicture}[scale=1, transform shape]
    \node[ndout] (X) at (0,0) {$X$};
...
\[ Y \Perp^d_{G_{\doit(I_Z,X,W)}^{\sm(L,R )}} I_Z \given X,Z,W,\]
in the CADMG:
\centerline{
\begin{tikzpicture}[scale=1, transform shape]
...
\[ Y \Perp^d_{G_{\doit(I_X,Z,W)}^{\sm(L,R )}} I_X \given Z,W,\]
in the CADMG:
\centerline{
\begin{tikzpicture}[scale=1, transform shape]
    \node[ndout] (X) at (0,0) {$X$};
...
\[ X \Perp^d_{G_{\doit(I_Z,W)}^{\sm(L,R )}} I_Z \given W,\]
in the CADMG:
\centerline{
\begin{tikzpicture}[scale=1, transform shape]
    \node[ndout] (X) at (0,0) {$X$};
...
\[ Y \Perp^d_{G_{\doit(I_Z,W)}^{\sm(L,R )}} I_Z \given X,Z,W,\]
in the CADMG:
\centerline{
\begin{tikzpicture}[scale=1, transform shape]
    \node[ndout] (X) at (0,0) {$X$};
