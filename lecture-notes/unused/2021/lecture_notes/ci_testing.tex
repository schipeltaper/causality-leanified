...
\end{itemize}
We will consider these questions only for the special case of discrete categorical variables, i.e., 
...
involving random and non-random variables.
...
\subsubsection{Marginal Independence for Categorical Random Variables}
...
We can also write the null hypothesis as $H_0 : \theta \in \Theta_0$.
...
$(X_n,Y_n) \sim P(X,Y \mid \theta)$ for all $n=1,\dots,N$, with the ``true'' parameter $\theta$
unknown.
...
The classical frequentist procedure for deciding between two hypotheses $H_0 : \theta \in \Theta_0$
and $H_1 : \theta \in \Theta\setminus\Theta_0$ is as follows.
One comes up with a \emph{test statistic} $T(D)$, which is a function of the data $D \sim P(D \mid \theta)$, 
whose value should help us distinguish between the two hypotheses. Then one chooses
a particular $p$-value threshold $\alpha \in (0,1)$. From
...
Often, the main desideratum is to control the Type I error (i.e., the error of incorrectly
...
as there we require both Type I error and Type II error (i.e., the error of incorrectly 
...
is also known as the \emph{$G$ test}. We start by writing down the likelihood of the data:
...
Maximizing the likelihood with respect to the parameters $\theta \in \Theta$, we obtain the well-known maximum likelihood estimator
...
This is just the product of two independent multinomial distributions with parameters
...
\begin{equation}\label{eq:G_test_statistic}
  G_N := 2 \log \frac{\sup_{\theta\in\Theta_0\cup\Theta_1} P((X_n,Y_n)_{n=1}^N \mid \theta)}{\sup_{\theta\in\Theta_0} P((X_n,Y_n)_{n=1}^N \mid \theta)} = 2 \sum_{x\in\C{X}} \sum_{y\in\C{Y}} N_{xy} \log \frac{N_{xy} N}{N_{x+} N_{+ y}}.
\end{equation}
...
The dimensionality of $\Theta$ is $|\C{X}||\C{Y}| - 1$, while
that of $\Theta_0$ is $(|\C{X}|-1) + (|\C{Y}|-1)$. The degrees
of freedom of the asymptotic chi-square distribution is the difference of the two, i.e.,
$\nu = (|\C{X}||\C{Y}|-1)   -   ((|\C{X}|-1) + (|\C{Y}|-1)) = (|\C{X}| - 1)(|\C{Y}| - 1)$.

An alternative proof will be provided later in a more general setting (Proposition~\ref{prp:ext_G_test_asymptotics_H0}).
\end{proof}
...
Indeed, if $\theta \in \Theta_0$, then $P(G_N \ge \chi_{\nu,1-\alpha}^2) \to \alpha$, for any $\alpha \in (0,1)$.
Since 
...
This is an estimator (the so-called ``plug-in estimator'') of the mutual information $I(X;Y)$:
\begin{equation*}\begin{split}
...
\end{split}\end{equation*}
With Jensen's inequality, one can show that $I(X;Y) \ge 0$, and that $I(X;Y) = 0 \iff X \Indep Y$.
Note further that the function $\Theta \to [0,\infty) : \theta \mapsto I(\theta)$ is continuous.
...
data set $(X_n,Y_n)_{n=1}^\infty$, where one rejects $H_0 : X \Indep Y$ if $G_N \ge \tau_N$ for some given
...
Let $\theta \in \Theta$. Since the $(X_n,Y_n)$ are assumed to be i.i.d., and 
...
$P(N_{xy} / N \to \theta_{xy}) = 1$.}
Hence $\hat\theta_{xy} \stackrel{a.s.}{\to} \theta_{xy}$. Hence, also
$\hat\theta_{+ x} \stackrel{a.s.}{\to} \theta_{+ x}$ and
$\hat\theta_{y +} \stackrel{a.s.}{\to} \theta_{y +}$.
...
Since a.s.\ convergence implies convergence in probability,
  $$\theta \in \Theta_1 \implies P(G_N < \tau_N) \to 0.$$
...
We conclude that 
  $$\theta \in \Theta_0 \implies P(G_N \ge \tau_N) \to 0,$$
...
that to bound Type I error a fixed critical value $\alpha$ suffices, but for consistency we
...
\subsubsection{Conditional Independence for Categorial Random Variables}
...
We formulate the null hypothesis $H_0 : X \Indep Y \given Z$ of independence
...
for all $z \in \C{Z}$ with $\theta_z > 0$; however, since the data will not convey any information on $\theta_{xy|z}$
for such $z$, this does not matter).
...
We can then also write the null hypothesis as $H_0 : \theta \in \Theta_0$ with
$\Theta_0 := \Theta_Z \times \Theta_{XY\given Z}^0$.
As alternative hypothesis we take that of dependence, i.e., $H_1 : X \nIndep Y \given Z$, or equivalently,
$H_1 : \theta \in \Theta_1$, where
$\Theta_1 := \Theta_Z \times \Theta_{XY\given Z}^1$ 
with $\Theta_{XY\given Z}^1 := \Theta_{XY\given Z} \setminus \Theta_{XY\given Z}^0$.
...
We again work out the details of the likelihood ratio test, and 
start by writing down the likelihood of the data:
\begin{equation*}\begin{split}
  P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta) 
...
for each $z \in \C{Z}$.
Maximizing the likelihood with respect to the parameters $\theta \in \Theta$, we obtain the maximum likelihood estimator
...
and $Y$:
\begin{equation*}\begin{split}
  \theta \in \Theta_0 \implies P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta) 
...
\frac{\sup_{\theta\in\Theta_0\cup\Theta_1} P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta)}{\sup_{\theta\in\Theta_0} P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta)}
  & = \frac{P((X_n,Y_n,Z_n)_{n=1}^N \mid \hat\theta)}{P((X_n,Y_n,Z_n)_{n=1}^N \mid \hat\theta^0)}
...
G_N := 2 \log \frac{\sup_{\theta\in\Theta_0\cup\Theta_1} P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta)}{\sup_{\theta\in\Theta_0} P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta)} = 2 \sum_{z\in\C{Z}} \sum_{x\in\C{X}} \sum_{y\in\C{Y}} N_{xyz} \log \frac{N_{xyz} N_{+ + z}}{N_{x+ z} N_{+ y z}}.
\end{equation}
...
The dimensionality of $\Theta$ is $|\C{Z}|(|\C{X}||\C{Y}| - 1) + (|\C{Z}|-1)$, while
that of $\Theta_0$ is $|\C{Z}|\big((|\C{X}|-1) + (|\C{Y}|-1)\big) + (|\C{Z}|-1)$. The degrees
of freedom of the asymptotic chi-square distribution is the difference of the two, i.e.,
$\nu = |\C{Z}|(|\C{X}| - 1)(|\C{Y}| - 1)$.
\end{proof}
...
where one rejects $H_0 : X \Indep Y \given Z$ if $G_N \ge \tau_N$ for some given
sequence of thresholds $\tau_N$.
...
Let $\theta \in \Theta$. Since $(X_n,Y_n,Z_n)$ are assumed to be i.i.d., and 
...
$\hat\theta_{+ x|z} \stackrel{a.s.}{\to} \theta_{+ x|z}$, and
$\hat\theta_{y +|z} \stackrel{a.s.}{\to} \theta_{y +|z}$.
