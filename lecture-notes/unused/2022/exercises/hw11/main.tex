...
\begin{enumerate}
\item ($5$ points) Given SCM: $M = \left\langle J=\emptyset, V=\{A, B, C, D, E, F\}, W=\{A', B', C', D', E', F'\}, \R^{12}, P, f \right\rangle$, where $P$ are six independent normal distributions with means and standard deviations $\mu_A, \sigma_A, \mu_B, ..., \sigma_F$, and
\[
	\begin{pmatrix}
		f_A(x, \varepsilon) \\ f_B(x, \varepsilon) \\ f_C(x, \varepsilon) \\ f_D(x, \varepsilon) \\ f_E(x, \varepsilon) \\ f_F(x, \varepsilon)
	\end{pmatrix}
	= 
	\begin{pmatrix}
...
\begin{enumerate}
	...
	\item\label{ex:acy_obs} Show that $M^{\text{acy}}$ is observationally equivalent to $M$.
	\item Show that $M^{\text{acy}}$ is not interventionally equivalent to $M$.
	\item\label{ex:sep} Show the following:
	\begin{enumerate}
...
\end{enumerate}
For \ref{ex:acy_obs} and \ref{ex:sep}, make the exercises without relying on results from the lecture notes that immediately prove te statement.

\begin{gradedsolution}
	\begin{enumerate}
		\item We have strongly connected component $\{A, B, C\}$. Applying Proposition \ref*{prp:linear_scms} with $L = \{A, B, C\}$,
		\begin{equation*}
		...
		\end{align*}
		Computing this explicitly, we find:
		\begin{figure}[H]
			\centering
			\frame{\includegraphics[width=.8\textwidth]{sol-hw11.png}}
		\end{figure}
		...
		\begin{center}
			...
			\end{tikzpicture}
		\end{center}
		\item This follows from the series of equivalences between $f$ and $\tilde{f}$ in the proof of Proposition \ref*{prp:scm_acyclification_properties}. Check this!
		\item E.g. intervene $X_A=x_a$. Then in $M$, $B$ is affected by $x_a$, but in $M^{\text{acy}}$, it is not.
		\item The first two are false. The third is true.
	\end{enumerate}
\end{gradedsolution}

\item ($3$ points) We will prove that the SCM $M$ of the equilibrium behaviour of a damped coupled harmonic oscillator in Example \ref*{ex:HarmonicOscillator} is simple:
...
\begin{enumerate}
	...
	\item Show that $M$ satisfies the matrix form as in Proposition \ref*{prp:linear_scms}.
	...
	\emph{Hint: you can use that the determinant of a tridiagonal matrix of the following form is given by the expression on the r.h.s.\footnote{Bonus points for who comes up with an elegant proof of this identity that does not rely on solving the recurrence relationship for the determinant of tridiagonal matrices.}:
	$$\det \begin{pmatrix}
		k_0+k_1 & -k_1       & & & \\
		-k_1     & k_1 + k_2 & -k_2       & & \\
				& -k_2       & k_2 + k_3 & \ddots  & \\
...
\begin{gradedsolution}
	\begin{enumerate}
		...
		\item For any $T \subseteq V$, the matrix $(1_{V}-B_{VV})_{(V \setminus T)(V \setminus T)} = 1_{V \setminus T}-B_{(V \setminus T)(V \setminus T)}$ is a block-diaginal matrix, where each block is either a diagonal or a tridiagonal matrix. (Check this!) The determinant of a block-diagonal matrix equals the product of the determinants of the constituent blocks. Using the same analysis as in the previous exercise, it follows that $\det(1_{V \setminus T}-B_{(V \setminus T)(V \setminus T)}) > 0$, hence $1_{V \setminus T}-B_{(V \setminus T)(V \setminus T)}$ is invertible. From Proposition \ref*{prp:linear_scms} we know that for every $T\subseteq V$ the SCM $M_{\doit (T)}$ is uniquely solvable, and from the definition of simple SCMs (Definition \ref*{def:simple_scm}) we conclude that $M$ is simple.
	\end{enumerate}
\end{gradedsolution}
	
\item (1 point) Consider the dynamical system and the SCM given in Example \ref*{ex:PriceSupplyDemandDynamical} (Price, supply and demand) of the lecture notes.

Show for which of the 8 subsets $T \subseteq \{D, S, R\}$, the intervened SCM $M_{\text{do}(T)}$ is uniquely solvable. You can use the condition for unique solvability of linear SCMs from Proposition \ref*{prp:linear_scms} of the lecture notes.
