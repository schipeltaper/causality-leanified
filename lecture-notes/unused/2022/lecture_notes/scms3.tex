...
%        \draw[arout] (X1) -- (X2);
      ...
      \begin{scope}[xshift=12cm]
        ...
        \draw[arout] (X2) -- (X3);
        \draw[arlat,bend left] (X2) edge (X3);
        \node at (2,-1) {$G(\tilde{M})^{\sm \{A,C\}}$};
      \end{scope}
    \end{tikzpicture}
  \end{center}
  \caption{The graphs of the interventionally equivalent SCMs $M$ (left) and $\tilde{M}$ (center) corresponding to Example~\ref{eg:interventionally_equivalent_but_different_graphs}. If we consider only the endogenous variables $X_1,X_2,X_3$ to be observed, and the other (exogenous random) variables to be latent, we can represent this with the two marginal graphs on the right.\label{fig:interventionally_equivalent_but_different_graphs}}
\end{figure}
...
\subsection{Interventional equivalence of acyclic SCMs and I(L)-CBNs}
...
We will show that in a precise sense, acyclic SCMs and IL-CBNs are equally expressive.
\begin{Def}
  Let $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ be a simple SCM and let $\tilde{M}$ be an IL-CBN with graph
  $\tilde{G} = (\tilde{J},\tilde{V}\dcup\tilde{U},\tilde{E})$, spaces $\tilde{\CX}_{\tilde v}$ for $\tilde{v} \in \tilde{V} \cup \tilde{U} \cup \tilde{J}$,
  and Markov kernels $$P_{\tilde{v}}\lp X_{\tilde{v}} | \doit \lp X_{\Pa^{\tilde{G}}}(\tilde{v}) \rp\rp.$$
  ...
  for any hard intervention $\doit(T)$ with $T \subseteq O$, the interventional Markov kernel $P_M(X_{O\sm T}|X_{J\cup T})$ of $M$ equals the interventional Markov kernel $P(X_{O\sm T}|X_{\tilde{J}\cup T})$ of $\tilde{M}$.
  %Special case $J = \tilde{J}$, $O = V \cup W$, $\tilde{V} = V \cup W$, $\tilde{U} = \emptyset$:
\end{Def}
\begin{Prp}
  \begin{enumerate}
%    \item Given an acyclic SCM $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$, we can construct an I-CBN $\tilde{M}$ with graph $G(M)$, spaces $\CX_v$ for $v \in V\cup W \cup J$, that is interventionally equivalent to $M$ (w.r.t.\ $O = V \cup W$).
%    \item Given an acyclic SCM $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ and an observed subset $O \subseteq V \cup W$, we can construct an IL-CBN $\tilde{M}$ with observed variables $\tilde{V} = O$, latent variables $W$, graph $G(M_{\sm (V\sm O)})^{\sm (W\sm O)}$, that is interventionally equivalent to $M$ w.r.t.\ $O$.
    \item Given an acyclic SCM $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ and an observed subset $O \subseteq V \cup W$, we can construct an IL-CBN $\tilde{M}$ (with observed output variables $\tilde{V} = O$, latent output variables $\tilde{U} = (V \cup W) \sm O$, input variables $J$, graph $G(M)$, and spaces $\CX_v$ for $v \in V\cup W \cup J$) that is interventionally equivalent to $M$ w.r.t.\ $O$.
    \item Given an IL-CBN 
      $\tilde{M} = \lp G^+=(J,(O,U),E^+), \; P\lp X_O,X_U| \doit\lp X_J \rp\rp \rp$
      with observed variables $O$, we can construct an acyclic SCM $M$ that is interventionally equivalent to it w.r.t.\ $O$.
  \end{enumerate}
\end{Prp}
\begin{proof}
  \begin{enumerate}
%    \item
%      Let $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ be an acyclic SCM with graph $G := G(M) = (J, V \cup W, E)$.
%      Define $\tilde{M}$ as the I-CBN with graph $G$, spaces $\CX_v$ for $v \in V\cup W \cup J$, and the following Markov kernels.
%      For $v \in V$, we write its structural equation as:
%      $$x_v = f_v(x_{\Pa^{G}(v)})$$
%      with $f_v : \CX_{\Pa^{G}(v)} \to \CX_v$,
%      where $v \notin \Pa^{G}(v)$ because the graph is acyclic. We then define the corresponding Markov kernel
%      \[ P_v\lp X_v|\doit\lp X_{\Pa^{G}(v)}\rp\rp := \delta_{f_v}(X_v|X_{\Pa^G(v)}).\]
%      The Markov kernels for $w \in W$ are defined as
%      \[ P_w\lp X_w|\doit\lp X_{\Pa^{G}(w)}\rp\rp := P_w(X_w),\]
%      where we note that $\Pa^G(w) = \emptyset$.
%      One can check that this I-CBN $\tilde{M}$ does the job.
%    \item
%      Let $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ be an acyclic SCM with graph $G := G(M) = (J, V \cup W, E)$
%      and let $O \subseteq V \cup W$. We construct the I-CBN $\hat{M}$ corresponding to the marginalization $M_{\sm (V\sm O)}$
%      as in point 1.
%      Finally, we marginalize out $W\sm O$ from I-CBN $\hat{M}$ to obtain an IL-CBN $\tilde{M}$.
%      This IL-CBN does the job, as one can check.
    \item
      Let $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ be an acyclic SCM with graph $G(M) = (J, V \cup W, E)$.
      Define $\tilde{M}$ as the IL-CBN with observed output variables $\tilde{V} = O$, latent output variables $\tilde{U} = (V \cup W) \sm O$, input variables $J$, graph $G^+:=G(M)$, spaces $\CX_v$ for $v \in V\cup W \cup J$, and the following Markov kernels.
      For $v \in V$, we write its structural equation as:
      $$x_v = f_v(x_{\Pa^{G^+}(v)})$$
      with $f_v : \CX_{\Pa^{G^+}(v)} \to \CX_v$,
      ...
      One can check that this IL-CBN $\tilde{M}$ does the job.
    \item
      Let 
      \[ \tilde{M} = \lp G^+=(J,O,U,E^+), \; P\lp X_O,X_U| \doit\lp X_J \rp\rp \rp\]
      be an IL-CBN.
      ...
      for some measurable function $R_v : \CX_{\bar v} \times \CX_{\Pa^{G^+}(v)} \to \CX_v$.
...
We will finish this section by showing that SCMs are simple if the interactions are sufficiently weak and smooth.
