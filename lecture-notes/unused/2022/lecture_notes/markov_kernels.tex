...
\begin{Def}[Transition measures and Markov kernels]
...
\begin{Not}[Transition measures and Markov kernels]
    \begin{enumerate}
        \item We often use suggestive notations as follows for transition measures and Markov kernels:
...
\begin{Lem}\label{transmeasure_is_twoargfunction}
    ...
    \begin{enumerate}
        \item a transition measure, i.e.\ a measurable map:
            \[ K(W|T):\, \Tcal \to \Msf(\Wcal),\qquad t \mapsto K(W|T=t).  \]
    ...
    \begin{enumerate}[label=\roman*)]
        \item For each $t \in \Tcal$ the map:
            ...
            is a finite measure (i.e.\ countably additive with $\tilde K(W \in \Wcal|T=t) < \infty$ for all $t \in \Tcal$\footnote{Note that for a transition measure the finite value $\tilde K(W \in \Wcal|T=t)$ can vary with $t \in \Tcal$. 
        ...
        \item For each $D \in \Bcal_\Wcal$ the map:
            \[ \tilde K(W \in D|T):\, \Tcal \to \R_{\ge 0},\quad t \mapsto \tilde K(W \in D|T=t) \]
...
\begin{Eg}[Exponential families as transition measures]
...
\begin{Rem}
    For absolute continuous transition measures $Q(W|T) \ll K(W|T)$ there exists by the Theorem of Radon-Nikodym, see Theorem \ref{thm:radon-nikodym} or \cite{Kle20} Cor.\ 7.34,
    ...
    This approach will be seen to also work for the construction of Doob-Radon-Nikodym derivatives for transition measures.
\end{Rem}
...
\begin{Def}[Null sets]
    Let $K(W|T):\, \Tcal \to \Msf(\Wcal)$ be a transition measure.
...
\begin{Lem}[Essential uniqueness of the Doob-Radon-Nikodym derivative]
    Let $\Tcal$, $\Wcal$ be measurable spaces and:
    \[ Q(W|T), K(W|T) :\, \Tcal \to \Msf(\Wcal), \]
    be two transition measures with $Q(W|T) \ll K(W|T)$ and let $g_1,g_2$ be two Doob-Radon-Nikodym derivatives. 
...
\begin{Thm}[Doob-Radon-Nikodym, see \cite{Del83} Thm.\ 58, \cite{Kle20} Ex.\ 11.17]
    ...
    be two transition measures. Assume that $\Wcal$ is a standard measurable space.\footnote{The proof shows that we actually only require that $\Bcal_\Wcal$ is \emph{countably generated}. \label{fn:count-generated}} Then the following two statements are equivalent:
    \begin{enumerate}
...
\begin{Rem}
    \begin{enumerate}
        ...
        \item With more technical conditions one could extend Theorem \ref{thm:doob-radon-nikodym} to work for $\sigma$-finite transition measures.
        \item A special case of Theorem \ref{thm:doob-radon-nikodym} would be to consider a statistical model $P(X|\Theta)$
            that is absolute continuous w.r.t.\ the Lebesgue measure $\lambda$, where $\Xcal=\R^D$.
            Then one gets a density $p$ w.r.t.\ $\lambda$, usually denoted $p(x|\theta)$, that is jointly measurable in $(x,\theta)$.
            Note that $\lambda$ is not finite, but it is equivalent, in terms of absolute continuity, to a probability measure, e.g.\ to any Gaussian probability distribution, to which Theorem \ref{thm:doob-radon-nikodym} then applies.
    \end{enumerate}
\end{Rem}
...
\begin{Thm}[Existence of the Doob-Radon-Nikodym derivative, see \cite{Del83} Thm.\ 58, \cite{Kle20} Ex.\ 11.17]
    Let $\Tcal$, $\Wcal$ be measurable spaces and:
    \[ K(W|T), \,Q(W|T):\, \Tcal \to \Msf(\Wcal),  \]
    be two transition measures. Assume that $\Wcal$ is a standard measurable space.\footref{fn:count-generated}
