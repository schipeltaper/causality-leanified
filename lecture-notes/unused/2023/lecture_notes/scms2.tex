...
%    \item If $M$ is uniquely solvable, then for all $\xJ\in\CXJ$, $M_{\doit(\XJ=\xJ)}$ is uniquely solvable.
    \item If $M$ is uniquely solvable, it has a unique Markov kernel 
      ...
      The distribution of any potential outcome $X^{\doit(\xJ)}_{V\cup W}$ for input $\xJ\in\XJ$ is given by $P_{M}(\XV,\XW \mid \doit(\XJ=\xJ))$.
  \end{enumerate}
\end{Thm}
\begin{proof}
  \begin{enumerate}
...
An SCM $M=\lt \Sin, \Send, \Srnd, \CX, P, f \rt$ is called \emph{linear} if all
...
Let $M=\lt \Sin, \Send, \Srnd, \CX, P, f \rt$ be a linear SCM.
Then, for any set $L \subseteq V$, the submodel
$M^{[L]}$ is uniquely solvable if and only if the matrices 
...
\begin{Def}\label{def:simple_scm}
  ...
  is called \emph{simple} if the intervened SCM
...
\begin{Rem}\label{rem:simple_scm}
  It suffices to check whether $M_{\doit(T)}$ is uniquely solvable for all $T \subseteq \Send$,
  since interventions on exogenous variables turn them into exogenous input variables, leaving
  the solution function invariant.
\end{Rem}
...
\begin{Cor}\label{cor:simple_scm}
  If $M$ is simple, then its Markov kernel $P_{M}(X_V, X_W \mid \doit(X_J))$ and all intervened Markov kernels
...
\end{Cor}
\begin{proof}
  This follows from Theorem~\ref{thm:scm_unique_solvability}.
\end{proof}
...
the class of simple SCMs is more expressive than the class of L-CBNs since it allows to model causal cycles, to some extent.

\subsection{Observable vs.\ Interventional Equivalence}
...
\begin{Def}
  ...
  \end{enumerate}
\end{Def}
In this section, we will discuss two important equivalence relations between SCMs.
\begin{comment}
\JorisTODO{The following notion has become redundant. It can be safely removed.}
...
\begin{Eg}\label{eg:self_cycle}
  The SCM with endogenous variable $X \in \RN$ and exogenous random variable $W \in \RN$ and structural equation
  $$X = W X + c$$
...
(the general definitions are provided in \cite{BFPM21} for SCMs without exogenous input variables).\footnote{\cite{BFPM21} also
define a stronger notion of equivalent SCMs which is useful when weakening the ``for all'' quantifier over values of
exogenous random variables by the ``for almost all'' quantifier. Since here we will only work with the ``for all'' quantifier, 
there is no need to discuss that equivalence relation here.}
...
Interventional equivalence is a stronger property than observational equivalence:
...
%    \item 
  Interventional equivalence of $M$ and $\tilde{M}$ w.r.t.\ $O$ implies observable equivalence of $M$ and $\tilde{M}$ w.r.t.\ $O$.
%  \end{enumerate}
\end{Prp}
\begin{proof}
%  \begin{enumerate}
%    \item
%       Suppose $M \equiv \tilde{M}$. Then for every $T \subseteq O$, $M_{\doit(T)} \equiv \tilde{M}_{\doit(T)}$.
%       Since equivalent SCMs have the same Markov kernels, $M_{\doit(T)}$ and $\tilde{M}_{\doit(T)}$
%       are observably equivalent w.r.t.\ $O \setminus T$ for every $T \subseteq O$.
%
%     \item 
  This is trivial (consider the intervention targeting $T = \emptyset$).
%  \end{enumerate}
\end{proof}
...
For certain parameter choices, they are observably equivalent.
%If $\mu = \tilde{\alpha}\tilde{\mu}$ and $\tilde{\mu} = \alpha \mu$ and
%  $\sigma^2 = \tilde{\beta}^2\tilde{\sigma}^2$ and $\beta \sigma^2 = \tilde{\beta}\tilde{\sigma}^2$ and $\beta^2 = \tilde{\sigma}^2$ then they are observably equivalent.
...
\begin{Def}\label{def:scm_marginalization}
  ...
  the submodel $M^{[L]}$ is uniquely solvable.
  Let $\tilde{g}_L : \CX_{\Sin} \times \CX_{V \sm L} \times \CXW \to \CX_L$
  be the unique solution function for $M^{[L]}$.
...
\begin{Not}\label{not:scm_marginalization}
  We will also use the notation $M_A := M_{\sm (V \sm A)}$ for a subset $A \subseteq V$ of endogenous variables
...
\begin{Lem}\label{lem:marginalization_preserves_unique_solvability}
  Let $M = \lt \Sin, \Send, \Srnd, \CX, P, f \rt$ be an SCM and $L \subseteq \Send$ such that $M^{[L]}$ is uniquely solvable.
  ...
  $M_{\setminus L}$ is $g_{V\sm L} = \pr_{V\sm L} \circ g$, where $g : \CX_J \times \CX_W \to \CX_V$ is the 
  unique solution function for $M$ and $\pr_K : \CXV \to \CX_K : x \mapsto x_K$ is the canonical projection on $K \subseteq V$.
\end{Lem}
\begin{proof}
  Let $K := V\sm L$ and let $\tilde{g}_L : \CX_{\Sin \cup K} \times \CXW \to \CX_L$
  be the unique solution function for $M^{[L]} = M_{\doit(K)}$.
...
\begin{Prp}\label{prp:marginalization_commutes}
  ...
  If $M^{[L_1]}$ is uniquely solvable, and
  $(M_{\setminus L_1})^{[L_2]}$ is uniquely solvable, then $M^{[L_1\cup L_2]}$ is uniquely solvable, and in that case it does not matter if we first marginalize over $L_1$ and then $L_2$, or both at once, i.e:
  $$(M_{\setminus L_1})_{\setminus L_2} = M_{\setminus (L_1\cup L_2)}.$$
\end{Prp}
\begin{proof}
  Write $K_1 = V \setminus L_1$.
  Let $\tilde{g} : \CX_{J\cup K_1} \times \CXW \to \CX_{L_1}$ be the unique solution function of $M^{[L_1]}$,
  i.e., for all $x \in \CX$:
  ...
  If $(M_{\setminus L_1})_{[L_1\cup L_2]}$ is uniquely solvable, it has a unique solution function
...
%    \iff &
  ...
  \end{split}\end{equation*}
  where in the first equivalence we used the unique solvability of $M^{[L_1]}$, in the second
  equivalence we used substitution, in the third equivalence we used the unique solvability of
  $(M_{\setminus L_1})^{[L_2]}$, in the fourth equivalence we used
  substitution again, and in the fifth equivalence we used the definition of $h$.
  Therefore, $h$ is the unique solution function for $M^{[L_1\cup L_2]}$, which must therefore
...
%  By Theorem~\ref{thm:scm_unique_solvability}, the Markov kernels $P_{M}(X_V, X_W \mid X_J)$ and $P_{M_K}(X_K, X_W \mid X_J)$ exist and are unique. Hence, the marginal Markov kernels $P_{M}(X_K,X_W \mid X_J)$ and $P_{M_K}(X_K,X_W \mid X_J)$ exist and are unique. 
