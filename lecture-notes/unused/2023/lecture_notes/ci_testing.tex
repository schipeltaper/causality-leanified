...
\JorisTODO{Updated notation: I now use $\Theta$ for the parameter variable and $\theta$ for its value and $\CX_\Theta$ for the parameter space.
Many of the ``$\theta$'' should become a $\Theta$, and $\hat\theta$ should become a $\hat\Theta$ because the estimator depends on the data and hence on $\omega$.}
...
\begin{itemize}
  ...
  \item \dots a transitional random variable is conditionally independent of a transitional random variable, given another transitional random variable?
\end{itemize}
...
involving transitional random variables (with ``purely'' random and ``purely'' non-random variables as special cases).
...
$(X_n,Y_n) \given \Theta \sim P(X,Y \mid \Theta)$ for all $n=1,\dots,N$, with the ``true'' parameter $\theta$
unknown.
In other words, we assume for the joint distribution on the observed data
$$P((X_n,Y_n)_{n=1}^N \mid \Theta) = \bigotimes_{n=1}^N P(X_n,Y_n \mid \Theta),$$
where each $P(X_n,Y_n \mid \Theta)$ is a copy of the Markov kernel $P(X,Y \mid \Theta)$.
...
One comes up with a \emph{test statistic} $T(D)$, which is a function of the data $D \sim P(D \mid \Theta)$, 
...
$$p(d) := \sup_{\theta\in\CX_\Theta^0} P(T(D) \ge T(d) \mid \theta).$$
...
Indeed, from the definition of the $p$-value it follows that:
  \[ \forall \alpha \in (0,1)\, \forall \theta \in \CX_\Theta^0:\;  P( p(D) \le \alpha \mid \theta) \le \alpha.\]
...
Maximizing the likelihood with respect to the parameters $\theta \in \CX_\Theta$, we obtain the well-known maximum likelihood estimator
...
\begin{equation*}\begin{split}
  \theta \in \CX_\Theta^0 \implies P((X_n,Y_n)_{n=1}^N \mid \theta) 
...
\frac{\sup_{\theta\in\CX_\Theta^0\cup\CX_\Theta^1} P((X_n,Y_n)_{n=1}^N \mid \theta)}{\sup_{\theta\in\CX_\Theta^0} P((X_n,Y_n)_{n=1}^N \mid \theta)}
...
\begin{equation}\label{eq:G_test_statistic}
  G_N := 2 \log \frac{\sup_{\theta\in\CX_\Theta^0\cup\CX_\Theta^1} P((X_n,Y_n)_{n=1}^N \mid \theta)}{\sup_{\theta\in\CX_\Theta^0} P((X_n,Y_n)_{n=1}^N \mid \theta)} = 2 \sum_{x\in\C{X}} \sum_{y\in\C{Y}} N_{xy} \log \frac{N_{xy} N}{N_{x+} N_{+ y}}.
\end{equation}
...
Indeed, if $\theta \in \CX_\Theta^0$, then $P(G_N \ge \chi_{\nu,1-\alpha}^2 \mid \theta) \to \alpha$, for any $\alpha \in (0,1)$.
Since 
...
\begin{equation*}\begin{split}
  ...
  & = \sum_{x\in\C{X}} \sum_{y\in\C{Y}} P(X=x,Y=y \mid \theta) \log \frac{P(X=x,Y=y \mid \theta)}{P(X=x \mid \theta) P(Y=y \mid \theta)} =: I(X;Y \mid \theta).
\end{split}\end{equation*}
With Jensen's inequality, one can show that $I(X;Y \mid \theta) \ge 0$, and that $I(X;Y \mid \theta) = 0 \iff X \Indep Y \mid \Theta=\theta$.
...
$P(N_{xy} / N \to \theta_{xy} \mid \theta) = 1$.}
...
Since a.s.\ convergence implies convergence in probability,
  $$\theta \in \CX_\Theta^1 \implies P(G_N < \tau_N \mid \theta) \to 0.$$
...
We conclude that 
  $$\theta \in \CX_\Theta^0 \implies P(G_N \ge \tau_N \mid \theta) \to 0,$$
...
$$\frac{P(X = x, Y = y, Z = z \mid \Theta = \theta)}{P(Z=z \mid \Theta=\theta)} = P(X = x, Y = y \mid Z=z, \Theta=\theta) = \theta_{xy|z}.$$
...
$$P(X=x,Y=y,Z=z \mid \Theta=\theta) = \theta_z \theta_{xy|z},$$
...
$(X_n,Y_n,Z_n) \sim P(X,Y,Z \mid \Theta)$ for all $n=1,\dots,N$, with the ``true'' parameter $\Theta \in \CX_\Theta$ unknown.
In other words, we assume for the joint distribution on the observed data
$$P((X_n,Y_n,Z_n)_{n=1}^N \mid \Theta) = \bigotimes_{n=1}^N P(X_n,Y_n,Z_n \mid \Theta),$$
where each $P(X_n,Y_n,Z_n \mid \Theta)$ is a copy of the Markov kernel $P(X,Y,Z \mid \Theta)$.
...
\begin{equation*}\begin{split}
  P((X_n,Y_n,Z_n)_{n=1}^N \mid \Theta) 
...
and $Y$:
\begin{equation*}\begin{split}
  \theta \in \CX_\Theta^0 \implies P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta) 
...
The likelihood ratio is obtained by dividing the likelihood for $\hat \theta$ by the likelihood for $\hat \theta^0$:
\begin{equation*}\begin{split}
\frac{\sup_{\theta\in\CX_\Theta^0\cup\CX_\Theta^1} P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta)}{\sup_{\theta\in\CX_\Theta^0} P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta)}
...
G_N := 2 \log \frac{\sup_{\theta\in\CX\Theta^0\cup\CX_\Theta^1} P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta)}{\sup_{\theta\in\CX_\Theta^0} P((X_n,Y_n,Z_n)_{n=1}^N \mid \theta)} = 2 \sum_{z\in\C{Z}} \sum_{x\in\C{X}} \sum_{y\in\C{Y}} N_{xyz} \log \frac{N_{xyz} N_{+ + z}}{N_{x+ z} N_{+ y z}}.
\end{equation}
...
\begin{equation*}\begin{split}
  ...
  & = \sum_{z\in\C{Z}} \sum_{x\in\C{X}} \sum_{y\in\C{Y}} P(X=x,Y=y,Z=z \mid \Theta=\theta) \log \frac{P(X=x,Y=y|Z=z,\Theta=\theta)}{P(X=x|Z=z,\Theta=\theta) P(Y=y|Z=z,\Theta=\theta)} \\
  & =: I(X;Y|Z).
\end{split}\end{equation*}
