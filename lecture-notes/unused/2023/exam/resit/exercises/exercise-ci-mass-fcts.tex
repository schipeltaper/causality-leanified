...
If you like, you can assume in this exercise that all spaces are countable discrete spaces and that all Markov kernels have mass functions
...
\begin{align}
    K(X,Y,Z|T) &= Q(X|Z) \otimes K(Y,Z|T),
\end{align}
where $K(Y,Z|T)$ is the marginal of $K(X,Y,Z|T)$. Multiplyting each side with $P(T)$ gives us:
\begin{align}
    P(X,Y,Z,T) & = K(X,Y,Z|T) \otimes P(T) \\
