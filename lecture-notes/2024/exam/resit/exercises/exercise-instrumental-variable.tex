...
\begin{tikzpicture}[scale=1, transform shape]
    ...
    \draw[arlout] (U) to (Y);
\end{tikzpicture}
\caption{A Directed Graph (DG) with observed variables $X$, $Y$, $Z$ and unobserved latent variable $U$. $Z$ and $U$ are exogenous random variables.}
\label{fig:instr-var-1}
\end{figure}

We now consider a simple SCM with graph depicted in Figure \ref{fig:instr-var-1}. We assume that the variables $X$, $Y$, $Z$ are observed real-valued random variables. $Z$ is an exogenous random variable, while $U$ is a real-valued unobserved exogenous random variable. 
Furthermore, we assume that the structural equation for $Y$ is (semi-)linear:
\begin{align*}
    Y &=  \beta \cdot X + h(U),
    %Y &:= f(X,U), & f(x,u) &:= \beta \cdot x + h(u),
\end{align*}
with some (potentially non-linear) measurable function $h$.
The coefficient $\beta$ here measures by how many units $Y$ would change if we would change $X$ by one unit via intervention. So $\beta$ quantifies the causal effect $P(Y|\doit(X))$ of $X$ on $Y$.

In the following we want to find different ways to determine $\beta$ by the knowledge of the graph $G$ in Figure \ref{fig:instr-var-1}, the use of the \emph{instrumental variable} $Z$, which we consider a continuously distributed real-valued random variable that correlates with $X$ (i.e.\ $\Cov(X,Z) \neq 0$), and the observed distribution $P(X,Y,Z)$.

\begin{enumerate}[resume*]
    \item Derive an expression for $\beta$ just in terms of the (observational) covariances between the variables $X$, $Y$, $Z$. Provide all arguments in detail. \\ 
      \emph{Hint: Make use of the known graphical structure, the Markov property, and the linearity of the covariance.}
 \score{4}
  \answer{%
    First note that $U \Perp^\sigma_G Z$. Indeed the only paths between $Z$ and $U$ are blocked:
    \begin{align*}
        Z \tuh X \hut U &&& \text{blocked at collider $X$}, \\
        Z \tuh X \tuh Y \hut U &&& \text{blocked at collider $Y$}, \\
        Z \tuh X \hut Y \hut U &&& \text{blocked at collider $X$}
    \end{align*}
    By the global Markov property this implies: $U \Indep_P Z$.
    By the separoid axioms this implies: $h(U) \Indep_P Z$.
    By the previous point this implies: $\Cov(h(U),Z) =0$.
    With these observations we get:
\begin{align*}
    &\Cov(Y,Z) \\
    &= \Cov(\beta \cdot X + h(U),Z) &&\text{definition of } Y \\
    &= \beta \cdot \Cov(X,Z) + \Cov(h(U),Z) && \text{linearity of } \Cov\\
    &= \beta \cdot \Cov(X,Z) && \text{as } \Cov(h(U),Z) =0.
\end{align*}
...
\begin{enumerate}[resume*]
    \item Derive an for expression for $\beta$ just in terms of conditional expectation values $\E[\_|W=0]$ 
      ...
      \emph{Hint: Make use of the known graphical structure, the Markov property, a separoid rule, and the linearity of conditional expectation.}
 \score{4}
\answer{%
  First note that we have: $h(U) \Indep_P Z$ and that $W = \I_{[Z > z_0]}$ is a measurable function of $Z$.
    By the separoid rules we get: $h(U) \Indep_P W$. 
    By the previous points we get: $\E[h(U)|W=w] = \E[h(U)]$ for $w=1,0$.
Together we then get that:
\begin{align*}
 & \E[Y|W=1] - \E[Y|W=0] \\
 &= \E[\beta \cdot X + h(U)|W=1] - \E[\beta \cdot X + h(U)|W=0] &&\text{definition of } Y \\
 &= \lp \beta \cdot \E[X|W=1] + \E[h(U)|W=1] \rp - \lp \beta \cdot \E[X|W=0] + \E[h(U)|W=0] \rp 
 && \text{linearity of } \E[\_|W=w]\\
 &= \beta \cdot \lp \E[X|W=1] - \E[X|W=0] \rp + \lp \E[h(U)|W=1]-\E[h(U)|W=0] \rp &&\text{rearrange terms} \\
 &= \beta \cdot \lp \E[X|W=1] - \E[X|W=0] \rp + \lp \E[h(U)]-\E[h(U)] \rp &&h(U) \Indep W, \text{ see above }\\
...
\end{align*}
\points{1pt for the independence argument $h(U) \Indep_P W$. (If $h(U) \Indep_P Z$ was not argued in the previous point those points could be gained here.) 2pt for the derivation. 1pt for final expression.   }
}
