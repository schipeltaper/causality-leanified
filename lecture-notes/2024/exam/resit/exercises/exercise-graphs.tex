...
\begin{enumerate}[label=\alph*)]
  \item When interpreted as the graph of a simple SCM $M$, what does the directed edge $B \tuh C$ represent? And the bidirected edge $A \huh B$? (You may assume that there is no selection bias)
    \score{2}
    \answer{$B \tuh C$ means that $B$ is a direct cause of $C$ according to $M$ (in words, this roughly means that if we intervene on $B, A, E, D, F$ then if we vary the value of $B$ while keeping those of $A, E, D, F$ fixed, the distribution of $C$ may vary). $A \huh B$ means (in the absence of selection bias) that there is a latent common cause (confounder) of $A$ and $B$.}
  \item Give the strongly connected components of $G$.
    \score{1}
    \answer{$\{A\}, \{B,C,D,E\}, \{F\}$.}
    ...
    \answer{%
        \begin{center}
            \begin{tikzpicture}[scale=1, transform shape]
                ...
                \draw[arlat] (A) to[out=3,in=150] (D);
                \draw[arlat] (A) to[out=3,in=-150] (E);
                \draw[arout] (A) to (B);
                \draw[arout] (A) to (C);
                \draw[arout] (A) to (D);
                \draw[arout] (A) to (E);
...
%    \item Draw the most informative (that is, with fewest circles as edge marks) $\sigma$-PAG that represents $G$.
%      \score{3}
%      \answer{%
%\begin{center}
%     \begin{tikzpicture}[scale=1, transform shape]
