...
**Exception — record-literal `def`s using `where { fields }`.** When a definition builds a structure via the `where` syntax (the `def_3_10` `hardInterventionOn`, `def_3_11` `nodeSplittingOn`, `def_3_13` `extendingCDMGsWith`, `def_3_14` `marginalize` shape — `def <name> ... : <StructType> where  field₁ := ...  field₂ := ...`), the entire `where` block is **part of the statement** (it *is* the definition — there is no separable proof body to factor out). The end marker MUST sit AFTER the last field of the `where` block, not after the type annotation. Failure mode this guards against: the website / for-website statement extractor reads only what's between the markers, so a marker placed before `where` renders the def as just its signature — `def hardInterventionOn (G : CDMG Node) (W : Finset Node) (hW : ...) : CDMG Node` — with no visible content. `def_3_10` shipped with this bug for months; the canonical example is `def_3_14` `marginalize` (markers wrap the full `def ... : CDMG Node where  J := G.J  V := G.V \ W  ... hL_irrefl := marginalize_hL_irrefl G W`). Apply this exception for any `def` whose body is `where { fields }` (typically structure / typeclass constructions); standard `def foo : T := ...term...` and theorem-shaped declarations keep the rule that the body sits below the end marker.
...
- **Lean: same-file marker blocks.** Wrap the original with `-- REFACTOR-BLOCK-ORIGINAL-BEGIN: <FinalName>` / `END`, and the replacement with `-- REFACTOR-BLOCK-REPLACEMENT-BEGIN: <FinalName> (was: refactor_<FinalName>)` / `END`. The replacement's declaration is named `refactor_<OriginalName>` so both can coexist while the build stays green (consumers keep using the old `<OriginalName>`). Cleanup deletes the ORIGINAL block, renames `refactor_<Name>` → `<Name>` *globally* (across every file the refactor touched, so cross-file references all flip together), and strips the markers.
  ...
  - **No-self-reference-by-unprefixed-name rule (load-bearing — failure mode that broke `eqViaNodeMap_injective` finalize across 5 files).** When the post-refactor `theorem refactor_<X>`'s proof needs to invoke the pre-refactor `theorem <X>` (e.g. to reuse a 4-conjunct image-equality proof as part of building a 5-conjunct InjOn-strengthened result), **the proof body must NOT call `<X>` by its unprefixed name**. That name resolves cleanly pre-cleanup (it refers to the ORIGINAL theorem in the same file), but after Phase 7a's rename `refactor_<X> → <X>` and the ORIGINAL deletion, the call becomes a self-recursive reference to the new theorem itself, and Lean's structural-recursion check fails to terminate. The cleanup script's Pass 1.8 catches this gate-time and refuses to finalize.

    Two safe patterns for reusing the pre-refactor proof:
    1. **Extract the original's proof as a private helper.** Inside the same file, add a `private theorem <X>_<descriptor> ...` (e.g. `<X>_imageEqs`, `<X>_preInjOn`, `<X>_unstrengthened`) with the OLD return type spelled out explicitly (don't use the refactored predicate). Paste the original's proof body verbatim. Call THAT helper from the new `theorem refactor_<X>`'s proof. The helper's name is unaffected by the cleanup rename, so it survives finalize.
    2. **Inline the original's proof.** Less DRY but valid. Paste the entire original proof body into the new theorem's proof, replacing the would-be self-recursive call.

    Litmus test before emitting `solved`: in every `REPLACEMENT` block named `<X>`, grep the body for the unprefixed identifier `<X>` (with `refactor_<X>` occurrences excluded, since those are the intentional self-references that will become `<X>` correctly). Any standalone `<X>` in the body is a self-recursion trap.
...
3. **(on `refactor_<name>`)** `python scaffold/scripts/phase3_solving/solve_chapter.py --data-path <refactor_data.json>`. Each refactor row runs through the existing manager/worker loop with the refactor briefing surfaced. Same-file Lean marker convention + tex twin convention apply. Manager-level `refactor` is BLOCKED here (nested refactor halts the run for human review). May take many sessions. **For unattended end-to-end runs**, use the wrapper `scaffold/scripts/run_refactor_pipeline.sh <refactor_data.json>` which chains solve → finalize → merge in a single backgrounded process (logs to `/tmp/refactor_pipeline_<name>.log`).
