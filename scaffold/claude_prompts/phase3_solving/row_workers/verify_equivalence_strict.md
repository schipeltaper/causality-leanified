...
- **CONTENT** -- changes what mathematical objects exist, what membership relations hold, what equalities or inequalities are forced, what cardinalities apply, what quantifiers strengthen or weaken. Examples:
  ...
  - **Loosening a quantifier's domain (taking 'any X' where the LN restricts to 'X with property P').** Symmetric to "Restricting a quantifier" above, and just as much a CONTENT change. Concrete example: the LN says *"Let `<` be a total order of `J ∪ V`"* and the Lean takes `lt : Node → Node → Prop` with no total-order hypothesis. The Lean is *strictly more general* than the LN — for any `lt` that isn't a total order, the Lean's def is well-defined but the LN's is not — so they're not the same mathematical object, even though they coincide on the LN's intended inputs. This is **CONTENT even when a design note documents the looseness** with a "constraint at use-site" rationale: the design note documents the divergence, doesn't undo it. The natural fix is either (a) carry the LN's hypothesis explicitly as `(h : <appropriate predicate>)` on the def's signature, or (b) refactor so the LN's hypothesis is encoded as a named wrapper predicate (`IsTotalOrder G lt`) that downstream consumers carry. Same applies to *every* LN premise clause that the Lean drops to a use-site obligation.
  - **Dropping LN-implicit set-typing / set-membership premises.** A subclass of "loosening a quantifier's domain" that is especially easy to miss because the LN rarely states these premises as separate `let` lines — they're baked into the LN's universe of discourse. The Lean's bare `Set Node` / `Node` / `Node × Node` argument is **strictly more general** than the LN's intended carrier, and that gap is **CONTENT**:
    - "A subset $A$ of $V$" / "$A, B, C \subseteq J \cup V$" / "any $W \subseteq V \cup J$" → the Lean must carry `(A : Set Node) (hA : A ⊆ ↑G.J ∪ ↑G.V)` (or `A ⊆ ↑G.V`, as the LN demands). A bare `A : Set Node` is CONTENT — the predicate admits "subsets" containing nodes the graph doesn't have.
    - "Let $v$ be a vertex of $G$" / "for $v \in G$" → `(v : Node) (hv : v ∈ G)`. A bare `v : Node` is CONTENT.
    - "Let $e$ be a directed edge of $G$" → `(e : Node × Node) (he : e ∈ G.E)`. A bare `e : Node × Node` is CONTENT.
    - **Side-condition aliases / graph-class restrictions.** If the LN defines a notation only when a side condition holds — e.g. "$A \perp_G B \mid C$ is defined when $G$ is a DMG ($J = \emptyset$)" — and the Lean alias does *not* carry that side condition as a premise, the alias compiles on a strictly larger class of inputs than the LN's notation is defined for. This is CONTENT — the LN's typed-concept distinction (σ vs iσ, DAG vs CADMG, etc.) is silently lost. The premise must be on the alias's type signature; a design comment isn't sufficient.

    Heuristic — read the def's signature *as if you didn't know what the LN was about* and ask: "could I instantiate this with an `A` containing nodes that aren't in the graph, or with a CDMG the LN never assigned this notation to, and have the encoding still parse?". If yes → CONTENT.
