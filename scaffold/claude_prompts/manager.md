...
A **refactor row** is a row that lives in a separate `refactor_data.json` (one per refactor target + all its transitive consumers, generated by `extras/initialize_refactor.py`). The orchestrator surfaces it via a `refactor: true` field on the row and a `## Refactor-row briefing -- READ FIRST` block in your row context. The block tells you:

- **Where the original lives** — the row's `main_lean_file` and `tex_file` point at the *existing* Lean / tex files (refactor work edits them in-place, not a new file). Read the original first; sometimes a refactor is a one-field tweak, sometimes a structural rewrite. Use it as inspiration, not scripture.
- **The same-file marker convention.** Wrap the original block with `-- REFACTOR-BLOCK-ORIGINAL-BEGIN: <FinalName>` / `END`, and the new block with `-- REFACTOR-BLOCK-REPLACEMENT-BEGIN: <FinalName> (was: refactor_<FinalName>)` / `END`. Use the *exact* marker format — the Phase 7 cleanup script greps for it. The replacement's declaration is named `refactor_<OriginalName>`; cleanup renames it back to `<OriginalName>` and deletes the ORIGINAL block, atomically across all affected files.
- **Build stays green** because consumers keep importing the original — your `refactor_X` is just an additional declaration the strict-equivalence solved-gate validates against the LN.
...
**On `solved`**, the orchestrator marks the row solved in `refactor_data.json` but **does not** regenerate the chapter / subsection aggregator `main.tex`, run the for-website worker, or touch the disprove-side cleanup. That work happens at Phase 7 cleanup, after every row in the refactor table is solved.
