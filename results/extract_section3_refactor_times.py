...
def write_latex(out_tex, sections, refactors, table, cols, labels,
    ...
    for row in table:
        cells = [r"\texttt{" + _tex_escape(row["ref"]) + "}", row["section"],
                 num(row.get("initial"))]
        for lab in labels:
            cells.append(num(row.get(lab)))
        cells.append(r"\textbf{" + f"{row_total[row['ref']]:,}" + "}")
        body.append("    " + " & ".join(cells) + r" \\")

    total_cells = [r"\textbf{TOTAL}", "", r"\textbf{" + f"{totals['initial']:,}" + "}"]
    for lab in labels:
        total_cells.append(r"\textbf{" + f"{totals[lab]:,}" + "}")
    total_cells.append(r"\textbf{" + f"{grand_total:,}" + "}")
    ...
    tex.append("% Cells are time_needed_to_solve in seconds; blank = the row")
    tex.append("% was not (re-)solved in that iteration.")
    tex.append(r"\begin{table}[htbp]")
    tex.append(r"  \centering")
    tex.append(r"  \caption{Time needed to solve (seconds) per row and per "
