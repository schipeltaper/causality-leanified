...
**Lean toolchain:** `leanprover/lean4:v4.29.1` (pinned in `lean-toolchain`); mathlib `v4.29.1` (pinned in `lakefile.toml`). `lake build` runs from the repo root.

**Everything Lean lives in `leanification/`** -- both the library root (`Causality.lean`) and every chapter's formalisations. No Lean files sit anywhere else in the repo.

Top-level layout:

```
.
├── README.md                                  scaffolding-readme stub from the cluster
├── claude.md                                  THIS FILE -- project rules, paradigm, commit/build workflow
├── lakefile.toml                              lake config: lib `Causality`, srcDir = leanification/, mathlib v4.29.1
├── lean-toolchain                             pinned Lean version: leanprover/lean4:v4.29.1
├── lake-manifest.json                         resolved dep tree (auto-generated by lake)
├── .gitignore                                 excludes .lake/, *.olean, build artefacts
│
├── extras/
│   └── explain_workflow.md                    end-to-end docs: every action, every worker, every key fn
│
├── lecture-notes/                             the source of truth -- the LN tex, *not* generated
│   └── lecture_notes/
│       ├── main.tex                           LN entry point; \input chain defines chapter order
│       ├── thm.tex                            \newtheorem environments (Def, Thm, Lem, ...)
│       ├── shorts.tex, shortsjoris.tex        macro shortcuts used by the LN
│       ├── graphs.tex                         chapter 3 (Graph Theory) -- currently the focused chapter
│       ├── experimental_causality.tex         chapter 1
│       ├── markov_kernels.tex                 chapter 2
│       └── ... (one .tex file per chapter)    every chapter's body lives here
│
├── leanification/                             ALL Lean formalisations live under this folder
│   ├── preamble.tex                           shared TeX preamble (bundled from LN; auto-managed)
│   ├── Causality.lean                         Lean library root; imports each Chapter<N>_<Title>
│   ├── Chapter3_GraphTheory.lean              chapter aggregator; auto-managed, imports every solved row's Lean file(s)
│   ├── refactors/                             one JSON per heavy redesign that's been triggered;
│   │   └── refactor_<title>.json              schema = data.json-like (rows = redesign step + every consumer to re-do)
│   ├── Chapter1_ExperimentalCausalDiscovery/  chapter folder
│   └── Chapter3_GraphTheory/                  chapter folder (active example)
...
└── scaffold/                                  the Python orchestrator + prompts + templates
    ...
    ├── prep_chapter.py                        STEP 1: spawn the find-defs-and-claims agent (marks LN tex in place)
    ├── initialize_chapter.py                  STEP 2: create chapter folder, data.json, wire Lean build, run fill_data, pick titles
    ├── solve_chapter.py                       STEP 3: the manager-loop orchestrator that drives rows to solved
    ├── present_chapter.py                     STEP 4 (placeholder): chapter presentation; to be filled in later
    ├── create_data.py                         row-parser; defines ACTIONS list (single source of truth); fill_data() called by init
    ├── main.py                                older sketch; currently unused
    ├── build_and_commit.sh                    ONLY sanctioned commit path: lake build then commit then push
    │
    ├── claude_prompts/                        prompts the orchestrator hands to spawned agents
    │   ├── manager.md                         the per-row manager prompt (read every turn)
    │   ├── chapter_setup/
    │   │   └── mark_definitions_and_claims_in_tex.md   prompt for the find-defs-and-claims agent (used by prep_chapter)
    │   └── row_workers/                       one prompt per worker the manager can dispatch
    │       ├── formalize_definition_in_lean.md         turn a defmark into Lean def(s)/structure(s)
    │       ├── formalize_claim_in_lean.md              turn a claimmark into a Lean theorem statement (with sorry)
    │       ├── write_tex_proof.md                      fill the row's proof tex (LN-first, then construct)
    │       ├── expand_tex_proof.md                     deepen one too-sketchy step in a tex proof
    │       ├── correct_tex_proof.md                    rewrite tex proof after leanification revealed a mistake
    │       ├── prove_claim_in_lean.md                  translate a verified tex proof into Lean tactics
    │       ├── add_design_choice_comments.md           write the *why* into the comment above each Lean declaration
    │       ├── review_design.md                        verifier (full LN context): is the Lean shape natural?
    │       ├── verify_equivalence.md                   verifier: does the Lean statement match the LN block exactly?
    │       ├── verify_tex_proof.md                     verifier: is the tex proof complete?
    │       ├── simplify_proof.md                       verifier (full LN context): is the proof unnecessarily complex?
    │       ├── verify_row_solved.md                    final-gate verifier before mark_solved
    │       ├── document_counterexample.md              (deprecated; disprove now uses the standard prove pipeline)
    │       ├── refactor_lean_code.md                   structural refactor of Lean code in a subsection
    │       └── plan_subtasks.md                        decompose a chunky row into ordered subtasks
    │
    └── tex_templates/                         templates the orchestrator instantiates per row
        ├── preamble.tex                       self-contained preamble (LN preamble + thm.tex + shorts macros)
        ├── main.tex.template                  subsection aggregator template (\subfile lines auto-filled)
        ├── def.tex.template                   per-def-row stub
        ├── claim_statement.tex.template       per-claim-row statement stub
        ├── claim_proof.tex.template           per-claim-row proof stub (includes statement on top)
        └── request_from_human.tex.template    per-chapter human-escalation channel
```

(Auto-built / gitignored: `.lake/` at the repo root holds Lake's build cache + fetched mathlib.)
...
## Scaffold workflow
For how the scaffold is built see **`extras/explain_workflow.md`**. That document contains an overview and exlpanation.
