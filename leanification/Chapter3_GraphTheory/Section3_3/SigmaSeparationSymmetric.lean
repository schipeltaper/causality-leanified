...
lemma WalkStep.reverse_reverse {G : CDMG Node} :
...
lemma WalkStep.isInto_reverse {G : CDMG Node} :
...
lemma Walk.length_reverse {G : CDMG Node} :
...
lemma Walk.vertices_reverse {G : CDMG Node} :
...
lemma Walk.nil_comp {G : CDMG Node} {u v : Node}
    (hv : u ∈ G) (q : Walk G u v) :
    (Walk.nil u hv).comp q = q := rfl

-- `q.comp nil = q` (modulo proof-irrelevance).  Used in the reverse_reverse
-- proof to absorb trailing nil-extensions.
lemma Walk.comp_nil {G : CDMG Node} :
...
lemma Walk.comp_reverse {G : CDMG Node} :
...
lemma Walk.reverse_reverse {G : CDMG Node} :
  ...
  | _, _, .cons mid s p' => by
      ...
      change (Walk.cons mid s.reverse.reverse (Walk.nil mid _)).comp p'.reverse.reverse =
        Walk.cons mid s p'
      rw [WalkStep.reverse_reverse, Walk.reverse_reverse p']
...
lemma Walk.hasBlockingLeftSlot_nil {G : CDMG Node} {v : Node}
    (hv : v ∈ G) (k : ℕ) :
    (Walk.nil v hv).HasBlockingLeftSlot k = False := by
  cases k <;> rfl

lemma Walk.hasBlockingRightSlot_nil {G : CDMG Node} {v : Node}
    (hv : v ∈ G) (k : ℕ) :
    (Walk.nil v hv).HasBlockingRightSlot k = False := by
  cases k <;> rfl

-- Recursive reduction lemmas (definitional).
lemma Walk.hasBlockingRightSlot_cons_succ {G : CDMG Node}
...
lemma Walk.hasBlockingRightSlot_at_length_false {G : CDMG Node} :
...
lemma Walk.hasBlockingRightSlot_out_of_range {G : CDMG Node} :
    ∀ {u v : Node} (p : Walk G u v) (k : ℕ),
...
lemma Walk.hasBlockingRightSlot_comp_eq_of_lt {G : CDMG Node} :
...
lemma Walk.hasBlockingRightSlot_comp_eq_at_length {G : CDMG Node}
    {b c mid mid₂ : Node} (s : WalkStep G b mid) (s' : WalkStep G mid mid₂)
    (r' : Walk G mid₂ c) :
    ∀ {a' : Node} (q : Walk G a' b),
...
lemma Walk.hasBlockingRightSlot_comp_eq_at_length_cons_nil {G : CDMG Node}
...
lemma Walk.hasBlockingRightSlot_comp_eq_at_length_singleton {G : CDMG Node}
...
lemma Walk.hasBlockingLeftSlot_out_of_range {G : CDMG Node} :
    ∀ {u v : Node} (p : Walk G u v) (k : ℕ),
...
lemma Walk.hasBlockingLeftSlot_comp_eq_of_lt {G : CDMG Node} :
    ∀ {u v w : Node} (q : Walk G u v) (r : Walk G v w) (k : ℕ),
      k ≤ q.length →
...
lemma Walk.hasBlockingLeftSlot_comp_eq_at_succ_length {G : CDMG Node}
...
lemma Walk.hasBlockingLeftSlot_reverse_eq_hasBlockingRightSlot {G : CDMG Node} :
...
-- The mirror invariance, derived from the above via `Walk.reverse_reverse`.
lemma Walk.hasBlockingRightSlot_reverse_eq_hasBlockingLeftSlot {G : CDMG Node}
  ...
  --        ↔ p.reverse.HasBlockingLeftSlot (p.length - k)
  rw [Walk.reverse_reverse, h_rev_len_eq] at h_iff
...
lemma Walk.vertices_length {G : CDMG Node} :
...
lemma Walk.vertices_reverse_getElem? {G : CDMG Node}
...
lemma Walk.isSigmaBlockedGiven_reverse {G : CDMG Node}
  ...
  · intro h_rev
    ...
    rw [Walk.reverse_reverse] at h_p
    exact h_p
