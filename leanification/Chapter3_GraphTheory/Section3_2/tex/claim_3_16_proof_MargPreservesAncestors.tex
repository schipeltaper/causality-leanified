...
\end{align*}
where the membership predicates $\Phi_E^{\sm W}$ and $\Phi_L^{\sm W}$ unfold as follows:
\begin{itemize}
    \item $\Phi_E^{\sm W}(\ul{v}, \ol{v})$ asserts the existence of a directed walk in $G$ (in the sense of def \ref{def:walks}, item~ii.) from $\ul{v}$ to $\ol{v}$ of length $n \ge 1$ whose intermediate vertices $w_1, \dots, w_{n-1}$ (if any) all lie in $W$, subject to the self-cycle restriction that $\ul{v} = \ol{v} \Rightarrow n \ge 2$. (The restriction is automatic in the witnessing argument because no walk of length~$1$ with $\ul{v} = \ol{v}$ has any intermediate vertex through which the path could pass; an honest witness for a $G^{\sm W}$-self-cycle $(\ul{v}, \ul{v}) \in E^{\sm W}$ requires a $G$-walk that traverses at least one $W$-vertex on its way back to $\ul{v}$, and is of length $\ge 2$ by construction.)
...
Apply $\Phi_E^{\sm W}$ to $\tilde{p}_j$: the walk $\tilde{p}_j$ witnesses the existential of $\Phi_E^{\sm W}(u_{c_j}, u_{c_{j+1}})$ directly --- length $\ge 1$ ✓, intermediates in $W$ ✓, and the self-cycle restriction $u_{c_j} = u_{c_{j+1}} \Rightarrow$ length $\ge 2$ is moot whenever $u_{c_j} \ne u_{c_{j+1}}$ and is satisfied otherwise because $\tilde{p}_j$ passes through at least one $W$-vertex in any nontrivial self-return (every consecutive pair $(c_j, c_{j+1})$ with $u_{c_j} = u_{c_{j+1}}$ and $c_{j+1} - c_j = 1$ arises from a directed self-loop $a_{c_j} = (u_{c_j}, u_{c_j}) \in E$ at the non-$W$ vertex $u_{c_j}$, contributing a zero-net step in the projection that can be omitted from the projected sequence without affecting the existential). By the contraction direction of $\Phi_E^{\sm W}$ (item~iii.\ of def \ref{def:G_marginalization}), $(u_{c_j}, u_{c_{j+1}}) \in E^{\sm W}$.
