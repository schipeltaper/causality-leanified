% Claim proof TWIN file for row claim_3_9 -- "SwigAcyclic".
% This is the refactor twin produced under refactor `cdmg_typed_edges` for
% the DEPENDENT row claim_3_9. The original pre-refactor proof lives at the
% sibling file `claim_3_9_proof_SwigAcyclic.tex` and remains untouched until
% the Phase 7 cleanup script renames this twin over it.
%
% Why a verbatim port suffices here: both sub-claims of claim_3_9 live
% entirely on the (J, V, E)-skeleton -- sub-claim (a) is acyclicity via
% directed walks on E, sub-claim (b) is a topological order built from
% Pa^G, which is defined from E alone -- and the bidirected-edge channel
% L plays no role in either sub-claim. This is recorded explicitly in the
% closing paragraph of the statement block below: "The bidirected-edge set
% $L$ plays no role in either sub-claim: acyclicity (def \ref{def-acylic})
% is defined via the non-existence of non-trivial directed walks (which
% use only $E$), and a topological order (def \ref{def-topological-order})
% is defined via the parent-set $\Pa^G$ (def \ref{def_3_5}, item~i., which
% is defined from $E$ alone)." The refactor `cdmg_typed_edges` changes
% only the encoding of L (from a Finset (Node x Node) carrier with a
% symmetry axiom to a quotient Finset (Sym2 Node) carrier with no
% symmetry axiom), and that change is invisible to the prose of this
% remark: the tex talks about CADMGs, edges, walks, topological orders,
% and parent sets abstractly and never references the Lean-side typing of
% L. Hence the statement block (synced verbatim with the canonical
% statement file `claim_3_9_statement_SwigAcyclic.tex`) and the proof
% body (ported verbatim from the existing pre-refactor proof file
% `claim_3_9_proof_SwigAcyclic.tex`) are identical to their pre-refactor
% counterparts. No `addition_to_the_LN` is attached to this row.
%
...
\begin{document}
% ref: claim_3_9 (proof twin for refactor `cdmg_typed_edges`, with statement restated for readability)
% title: SwigAcyclic
\def\rowref{claim\_3\_9}
\phantomsection\label{claim_3_9}

% --- statement (restated, synced verbatim with claim_3_9_statement_SwigAcyclic.tex) -----
...
% --- proof (ported verbatim from claim_3_9_proof_SwigAcyclic.tex) -----
\begin{proof}
