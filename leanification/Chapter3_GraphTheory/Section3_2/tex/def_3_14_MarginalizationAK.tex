...
\begin{Def}[Marginalization a.k.a.\ latent projection on CDMGs]\label{def:G_marginalization}
    ...
    \begin{enumerate}[label=\roman*.)]
        ...
        \item \emph{Directed edges.} The directed-edge set is the set of pairs witnessed by a directed walk in $G$ whose intermediate vertices lie in $W$, subject to the length restriction described below:
            \[
            ...
            \begin{enumerate}[label=(\alph*)]
                ...
                \item \emph{Self-cycle length restriction.} If $\ul{v} = \ol{v}$ then $n \ge 2$ (equivalently, the witnessing walk must contain at least one intermediate vertex $w_i \in W$). When $\ul{v} \neq \ol{v}$ no such restriction applies: the length-$1$ case $n = 1$ (a direct edge $(\ul{v}, \ol{v}) \in E$, with no intermediates) is admitted.
            \end{enumerate}
            In words: $\Phi_E(\ul{v}, \ol{v})$ asserts the existence of a directed walk in $G$ from $\ul{v}$ to $\ol{v}$, in the sense of def \ref{def:walks}, item~ii., of length $n \ge 1$, all of whose intermediate vertices $w_1, \ldots, w_{n-1}$ (if any) lie in $W$; clause~(d) imposes the additional self-cycle exclusion that, when $\ul{v} = \ol{v}$, a length-$1$ witness is not sufficient.\footnote{This clause may introduce self-cycles $v \tuh v$ into $E^{\sm W}$: namely, whenever $G$ contains a directed walk of length $\ge 2$ from $v$ back to $v$ with all intermediate vertices in $W$. However, the pre-existing length-$1$ edge $(v, v) \in E$ alone (with no intermediates through $W$) does \emph{not} on its own justify membership of $(v, v)$ in $E^{\sm W}$.}

            \emph{No $\ul{v} \neq \ol{v}$ constraint.} The condition $\ul{v} \neq \ol{v}$ is \emph{not} imposed in the set-builder for $E^{\sm W}$. Self-cycles $(v, v) \in E^{\sm W}$ are admitted exactly under the witness conditions above, namely via a directed walk of length $\ge 2$ through $W$ that returns to $v$.
    ...
    \begin{itemize}
        \item Directed self-cycles $v \tuh v$ may appear in $E^{\sm W}$, but \emph{only} when witnessed by a directed walk of length $\ge 2$ through $W$ (per clause (iii)(d)); a pre-existing length-$1$ edge $(v, v) \in E$ alone is insufficient.
