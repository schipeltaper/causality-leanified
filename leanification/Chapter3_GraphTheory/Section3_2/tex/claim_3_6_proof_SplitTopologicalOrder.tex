% Claim proof TWIN file for row claim_3_6 -- "SplitTopologicalOrder".
% This is the refactor twin produced under refactor `cdmg_typed_edges` for
% the DEPENDENT row claim_3_6. The original pre-refactor proof lives at the
% sibling file `claim_3_6_proof_SplitTopologicalOrder.tex` and remains
% untouched until the Phase 7 cleanup script renames this twin over it.
%
% Why a verbatim port suffices here: both sub-claims of claim_3_6 live
...
% L plays no role in either sub-claim (see the "Role of the CADMG
% (acyclicity) precondition" paragraph of the statement block below). The
% refactor `cdmg_typed_edges` changes only the encoding of L (from a
% Finset (Node x Node) carrier with a symmetry axiom to a quotient
% Finset (Sym2 Node) carrier with no symmetry axiom), and that change is
% invisible to the prose of this remark: the tex talks about CADMGs,
% edges, walks, topological orders, and parent sets abstractly and never
% references the Lean-side typing of L. Hence the statement block (synced
% verbatim with the canonical statement file
% `claim_3_6_statement_SplitTopologicalOrder.tex`) and the proof body
% (ported verbatim from the existing pre-refactor proof file
% `claim_3_6_proof_SplitTopologicalOrder.tex`) are identical to their
% pre-refactor counterparts. No `addition_to_the_LN` is attached to this
% row.
%
...
\begin{document}
% ref: claim_3_6 (proof twin for refactor `cdmg_typed_edges`, with statement restated for readability)
% title: SplitTopologicalOrder
\def\rowref{claim\_3\_6}
\phantomsection\label{claim_3_6}

% --- statement (restated, synced verbatim with claim_3_6_statement_SplitTopologicalOrder.tex) -----
...
% --- proof (ported verbatim from claim_3_6_proof_SplitTopologicalOrder.tex) -----
\begin{proof}
